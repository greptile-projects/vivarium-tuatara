...
export function RepositoryWorkspace({ compact = false }: { compact?: boolean }) {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    try { const result = await api<{ repositories: Repository[] }>("/repositories?limit=100", {}, token); setRepositories(result.repositories); }
