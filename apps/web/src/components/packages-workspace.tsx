...
export function PackagesWorkspace() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <p className="text-sm text-[var(--muted)]">Loading package evidence…</p> : filtered.length === 0 ? <Card className="p-8 text-center text-sm text-[var(--muted)]">No authorized package versions match this search.</Card> : <div className="grid gap-4">{filtered.map((item) => <Card key={item.id} className="p-5">
      ...
      <div className="mt-3"><Button variant="secondary" onClick={() => void loadConsumers(item)}>Show authorized consumers</Button>{consumers[`${item.name}@${item.version}`] && <div className="mt-3 text-xs text-[var(--muted)]">{consumers[`${item.name}@${item.version}`].length ? consumers[`${item.name}@${item.version}`].map((consumer) => <Link key={consumer.inventory.id} className="mr-3 text-[var(--brand)] hover:underline" href={`/repositories/${consumer.inventory.repository_id}/dependencies`}>{consumer.inventory.repository_id.slice(0, 8)} · {consumer.current ? "current" : "stale revision"} · {consumer.deployments.filter((value) => value.current).length} running</Link>) : "No visible consumers use this exact version."}</div>}</div>
    </Card>)}</div>}
  </div>;
}
