...
type Alert = {
  ...
  responder_load_consents?: Array<{ id: string; responder_id: string; interruption_minutes: number; created_at: string }>;
};
...
function ScopedResponsePoliciesWorkspace({
  ...
  async function reviewOutcome(alert: Alert) { if (!token) return; const classification = window.prompt("Outcome: actionable, false_positive, duplicate, needs_information, or incident_candidate", alert.workspace.classification ?? "actionable"); const rationale = window.prompt("Attributable outcome rationale"); if (!classification || !rationale) return; const routingAction = window.prompt("Routing containment: pause, activate_backup, resume, or leave blank", ""); const consent = alert.responder_load_consents?.at(-1); try { await api(`/repositories/${repositoryID}/response-alerts/${alert.id}/outcomes`, { method: "POST", body: JSON.stringify({ request_id: crypto.randomUUID(), classification, rationale, routing_action: routingAction || "", interruption_minutes: consent?.interruption_minutes ?? 0, responder_load_consent_id: consent?.id ?? "", agent_cost: alert.workspace.investigations.reduce((sum, item) => sum + item.budget, 0) }) }, token); setOutcomes(await api<OutcomeReport>(`/repositories/${repositoryID}/response-outcomes`, {}, token)); setError(""); } catch (x) { setError(x instanceof Error ? x.message : "Outcome review could not be retained"); } }
  async function consentResponderLoad(alert: Alert) { if (!token) return; const minutes = Number(window.prompt("Interruption minutes you consent to include in this alert outcome", "0") || 0); if (minutes < 1) return; try { const out = await api<Alert>(`/repositories/${repositoryID}/response-alerts/${alert.id}/responder-load-consents`, { method: "POST", body: JSON.stringify({ request_id: crypto.randomUUID(), interruption_minutes: minutes }) }, token); setAlerts(values => values.map(value => value.id === out.id ? out : value)); setError(""); } catch (x) { setError(x instanceof Error ? x.message : "Responder load consent could not be retained"); } }
