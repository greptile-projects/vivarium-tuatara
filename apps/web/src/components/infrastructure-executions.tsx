...
function governedWorkHref(
  repositoryID: string,
  response: Execution["drift_responses"][number],
) {
  switch (response.resource_kind) {
    case "issue":
      return `/repositories/${repositoryID}/issues/${response.resource_id}`;
    case "proposal":
    ...
    case "pull_request":
      return `/pulls/${repositoryID}/${response.resource_id}`;
    default:
      return null;
  }
}
      ...
      {items.map((x) => (
        <Card key={x.id} className="p-5">
          ...
          {x.drift_responses?.map((response) => {
            const href = governedWorkHref(repositoryID, response);
            return <div key={response.id} className="mt-3 rounded-lg border p-3">
              <div className="flex flex-wrap gap-2">
              ...
              <p className="text-xs text-[var(--muted)]">
                {href ? <Link className="font-semibold text-[var(--brand)] hover:underline" href={href}>Governed work {response.resource_id}</Link> : <>Governed work {response.resource_id}</>} · finding {response.finding_id}
              </p>
            </div>;
          })}
        </Card>
