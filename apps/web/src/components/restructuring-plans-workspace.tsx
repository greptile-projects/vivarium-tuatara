...
type Candidate = {
  ...
  gap_decisions?: { gap_kind:string; resource_id:string; decision:string; rationale:string; actor_id:string }[];
  rehearsals?: {
    id: string;
    state: string;
    cost_units: number;
...
export function RestructuringPlansWorkspace({
  ...
  async function decideGap(p:Plan,c:Candidate,g:NonNullable<Candidate["gaps"]>[number],decision:string){setSaving(true);try{const out=await api<Plan>(`/repositories/${repositoryID}/restructuring-plans/${p.id}/candidate-sets/${c.id}/gap-decisions`,{method:"POST",body:JSON.stringify({expected_version:p.version,decision:{request_id:crypto.randomUUID(),gap_kind:g.kind,resource_id:g.resource_id,decision,rationale:`${decision.replaceAll("_"," ")} under the retained owner boundary; the original gap remains visible`}})},token);setItems(x=>x.map(y=>y.id===out.id?out:y));setError("")}catch(x){setError(x instanceof Error?x.message:"Candidate gap decision could not be retained.")}finally{setSaving(false)}}
        ...
        <Card key={p.id}>
          ...
          {p.candidate_sets?.map((candidate) => (
            <section key={candidate.id} className="mt-2 rounded border p-3 text-sm">
              ...
              {candidate.gaps?.map((gap, index) => { const decided=candidate.gap_decisions?.find(x=>x.gap_kind===gap.kind&&x.resource_id===gap.resource_id),structural=["path_collision","cross_repository_link"].includes(gap.kind); return <div key={`${gap.kind}-${index}`} className="mt-2 text-[var(--danger)]"><p>{gap.kind}: {gap.summary} Decision: {gap.required_decision}</p>{decided?<p className="text-[var(--foreground)]">Dispositioned as {decided.decision.replaceAll("_"," ")} by {decided.actor_id}: {decided.rationale}</p>:structural?<p>Correct the candidate material and assemble a successor; this structural gap cannot be waived.</p>:<div className="flex gap-2"><Button type="button" variant="secondary" disabled={saving} onClick={()=>void decideGap(p,candidate,gap,"recreate")}>Recreate through ordinary work</Button>{gap.state==="inaccessible"&&<Button type="button" variant="secondary" disabled={saving} onClick={()=>void decideGap(p,candidate,gap,"retain_external")}>Retain under current owner</Button>}</div>}</div>})}
