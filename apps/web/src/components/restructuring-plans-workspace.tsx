        ...
        <Card key={p.id}>
          ...
          {p.candidate_sets?.map((candidate) => (
            <section key={candidate.id} className="mt-2 rounded border p-3 text-sm">
              ...
              {candidate.gaps?.map((gap, index) => { const decided=candidate.gap_decisions?.find(x=>x.gap_kind===gap.kind&&x.resource_id===gap.resource_id); return <div key={`${gap.kind}-${index}`} className="mt-2 text-[var(--danger)]"><p>{gap.kind}: {gap.summary} Decision: {gap.required_decision}</p>{decided?<p className="text-[var(--foreground)]">Dispositioned as {decided.decision.replaceAll("_"," ")} by {decided.actor_id}: {decided.rationale}</p>:gap.kind!=="path_collision"&&<div className="flex gap-2"><Button type="button" variant="secondary" disabled={saving} onClick={()=>void decideGap(p,candidate,gap,"recreate")}>Recreate through ordinary work</Button><Button type="button" variant="secondary" disabled={saving} onClick={()=>void decideGap(p,candidate,gap,"retain_external")}>Retain under current owner</Button></div>}</div>})}
