...
import { AccountControl } from "./auth";
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[15.5rem_minmax(0,1fr)]">
    ...
    <div className="min-w-0 lg:col-start-2">
      <header className="sticky top-0 z-10 flex h-16 items-center gap-3 border-b border-[var(--line)] bg-[rgb(251_252_248_/_0.88)] px-4 backdrop-blur-xl sm:px-6 lg:px-8">
        ...
        <Link href="/repositories" className="hidden min-h-9 items-center justify-center gap-2 rounded-lg border border-[var(--line-strong)] bg-white px-3.5 text-sm font-semibold transition hover:bg-[#f8faf7] sm:inline-flex"><Icons.Plus size={16}/>Create</Link>
        <Link href="/inbox" aria-label="Open inbox" className="grid size-9 place-items-center rounded-lg text-[var(--muted)] hover:bg-black/[.04] hover:text-[var(--ink)]"><Icons.Bell size={18}/></Link>
        <AccountControl />
      </header>
