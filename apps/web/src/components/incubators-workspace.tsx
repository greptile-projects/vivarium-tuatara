...
export function IncubatorsWorkspace() {
  ...
  async function publishLaunch(e: FormEvent<HTMLFormElement>) {
    e.preventDefault(); if (!token || !current) return; setPending(true); setError("");
    const f = new FormData(e.currentTarget); const audience = String(f.get("launch_audience"));
    try { const x = await api<Incubator>(`/incubators/${current.id}/launches`, { method:"POST", body:JSON.stringify({ expected_version:current.version, launch:{ readiness_id:f.get("launch_readiness"), audience, artifacts:lines(f.get("launch_artifacts")).map(v=>{const [kind,repository_id,resource_id,revision]=v.split("|").map(s=>s.trim()); return {kind,repository_id:repository_id||undefined,resource_id,revision:revision||undefined,audience};}) } }) }, token); replaceCurrent(x); e.currentTarget.reset(); }
    catch(reason){setError(reason instanceof Error?reason.message:"Unable to publish launch.");} finally{setPending(false);}
  }
  async function observeLaunch(e: FormEvent<HTMLFormElement>, launchId:string, launchVersion:number) {
    e.preventDefault(); if (!token || !current) return; setPending(true); const f=new FormData(e.currentTarget);
    try { const x=await api<Incubator>(`/incubators/${current.id}/launches/${launchId}/observations`,{method:"POST",body:JSON.stringify({expected_version:current.version,launch_version:launchVersion,observation:{kind:f.get("observation_kind"),repository_id:f.get("observation_repository")||undefined,resource_id:f.get("observation_resource"),summary:f.get("observation_summary")}})},token); replaceCurrent(x); e.currentTarget.reset(); } catch(reason){setError(reason instanceof Error?reason.message:"Unable to retain observation.");} finally{setPending(false);}
  }
  async function transitionLaunch(e: FormEvent<HTMLFormElement>, launchId:string, launchVersion:number) {
    e.preventDefault(); if(!token||!current)return; setPending(true); const f=new FormData(e.currentTarget);
    try{const x=await api<Incubator>(`/incubators/${current.id}/launches/${launchId}/transition`,{method:"POST",body:JSON.stringify({expected_version:current.version,launch_version:launchVersion,transition:{disposition:f.get("disposition"),target_resource_id:f.get("target_resource")||undefined,rationale:f.get("transition_rationale"),resolved_resources:lines(f.get("resolved_resources")),resolved_obligations:lines(f.get("resolved_obligations"))}})},token);replaceCurrent(x);}catch(reason){setError(reason instanceof Error?reason.message:"Unable to transition stewardship.");}finally{setPending(false);}
  }
  ...
  async function decideReadiness(e: FormEvent<HTMLFormElement>, readinessId:string, readinessVersion:number) {
    ...
    if(kind==="exception"&&(!expiry||Number.isNaN(expiryDate.getTime()))){setError("Exception expiry is required and must be within 30 days.");setPending(false);return;}
    try {replaceCurrent(await api<Incubator>(`/incubators/${current.id}/launch-readiness/${readinessId}/decisions`,{method:"POST",body:JSON.stringify({expected_version:current.version,readiness_version:readinessVersion,decision:{dimension:f.get("dimension"),kind,rationale:f.get("rationale"),follow_up_work:kind==="exception"?f.get("follow_up_work"):undefined,expires_at:kind==="exception"?expiryDate.toISOString():undefined}})},token));}
        ...
        <div className="grid gap-5 lg:grid-cols-[22rem_1fr]">
          ...
          {current ? (
            <Card className="p-6">
              ...
              <h3 className="mt-8 font-semibold">Launch and continuing stewardship</h3>
              <p className="mt-1 text-sm text-[var(--muted)]">Publish the ready project boundary, learn from real operational evidence, and make its lasting ownership explicit.</p>
              <div className="mt-3 space-y-3">{(current.project_launches??[]).map(p=><article key={p.id} className="rounded-lg border p-4"><div className="flex flex-wrap gap-2"><Badge tone={p.transition?"success":undefined}>{p.transition?.disposition.replaceAll("_"," ")??"operating"}</Badge><Badge>{p.audience}</Badge><Badge>{p.observations.length} observations</Badge></div><div className="mt-3 flex flex-wrap gap-2">{p.artifacts.map(a=><Badge key={`${a.kind}:${a.resource_id}`}>{a.kind.replaceAll("_"," ")} · {a.resource_id}</Badge>)}</div>{p.observations.map(o=><p key={o.id} className="mt-2 text-sm"><strong>{o.kind.replaceAll("_"," ")}</strong> · {o.summary}</p>)}{!p.transition&&<><form onSubmit={e=>void observeLaunch(e,p.id,p.version)} className="mt-4 grid gap-3 sm:grid-cols-2"><Select name="observation_kind" label="Learning signal" values={["adoption","support","reliability","cost","success_measure","feedback"]}/><Field name="observation_resource" label="Existing evidence ID"/><Field name="observation_repository" label="Repository ID" optional/><Area name="observation_summary" label="What the evidence shows"/><Button disabled={pending}>Retain observation</Button></form><form onSubmit={e=>void transitionLaunch(e,p.id,p.version)} className="mt-4 grid gap-3 sm:grid-cols-2"><Select name="disposition" label="Stewardship disposition" values={["organization_initiative","experimental","merged","archived"]}/><Field name="target_resource" label="Initiative or merged-project ID" optional/><Area name="transition_rationale" label="Evidence-backed rationale"/><Area name="resolved_resources" label="Resolved resources" hint="Required for archive; one per line"/><Area name="resolved_obligations" label="Resolved obligations" hint="Required for archive; one per line"/><Button disabled={pending}>Record transition</Button></form></>}</article>)}</div>
              {(current.project_launches??[]).length===0&&current.launch_readiness.some(r=>r.ready)&&<form onSubmit={publishLaunch} className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold">Ready assessment<select name="launch_readiness" className="mt-2 min-h-10 w-full rounded-lg border bg-white px-3 font-normal">{current.launch_readiness.filter(r=>r.ready).map(r=><option key={r.id} value={r.id}>{r.id}</option>)}</select></label><Select name="launch_audience" label="Declared audience" values={["experimental","limited","public"]}/><Area name="launch_artifacts" label="Published resources" hint="kind | repository ID | resource ID | optional 40-character revision. Include release, documentation, contributor opportunity, environment, and package or API contract."/><Button disabled={pending} className="sm:col-span-2">Publish first launch</Button></form>}
            </Card>
          ) : (
