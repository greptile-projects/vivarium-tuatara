...
type Analysis = { id:string; experiment_version:number; run_id:string; run_version:number; threshold_reason:string; segment_effects:{segment:string;exposures:Record<string,number>;metric_values:Record<string,number>;uncertainty:Record<string,number>}[]; exclusions:string[]; guardrail_outcomes:string[]; interpretation:string; interpreted_by_type:string; interpreted_by_id:string; uncertainty:string; dissent:string[]; created_by:string };
type OutcomeDecision = { id:string; version:number; experiment_version:number; analysis_id:string; decision:string; variant_key?:string; rationale:string; cleaned_up:boolean; tasks:{id:string;kind:string;title:string;required:boolean;status:string;evidence?:{pull_request_id:string;kind:string;resource_id:string};completed_by?:string}[] };
type RunAttempt = {
...
export function ProductExperimentsWorkspace({
  ...
  return (
    <div className="space-y-6">
      ...
      {selected && <ExperimentOutcome experiment={selected} userID={user?.id ?? ""} act={runAction} />}
      <section>
...
function pairs(v:string){return Object.fromEntries(lines(v).map(x=>{const [k,n]=x.split(":");return [k.trim(),Number(n)]}))}
function ExperimentOutcome({experiment,userID,act}:{experiment:Experiment;userID:string;act:(path:string,body:unknown)=>Promise<void>}) {
  const run=experiment.run_attempts?.at(-1), analysis=experiment.analyses?.at(-1), decision=experiment.outcome_decisions?.at(-1);
  const taskKinds=(choice:string)=>choice==="adopt_variant"?["rollout","remove_variants","remove_targeting","revoke_credentials","stop_collection","review","release","deployment"]:choice==="retain_control"?["rollback","remove_variants","remove_targeting","revoke_credentials","stop_collection","review","release","deployment"]:["follow_up","remove_variants","remove_targeting","revoke_credentials","stop_collection","review"];
  return <Card className="p-5"><h2 className="font-semibold">Decision and supported-system cleanup</h2><p className="mt-1 text-sm text-[var(--muted)]">Once evidence or a stop condition is reached, freeze the analysis, retain dissent, choose an outcome, and prove every obsolete flag, rule, credential, and collection path is retired.</p>
    {run&&!analysis&&<form className="mt-4 grid gap-3 md:grid-cols-3" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget),interpreter=value(f,"interpreter_type");void act("analyses",{analysis:{run_id:run.id,run_version:run.version,segment_effects:[{segment:value(f,"segment"),exposures:pairs(value(f,"exposures")),metric_values:pairs(value(f,"effects")),uncertainty:pairs(value(f,"segment_uncertainty"))}],exclusions:lines(value(f,"exclusions")),guardrail_outcomes:lines(value(f,"guardrails")),interpretation:value(f,"interpretation"),interpreted_by_type:interpreter,interpreted_by_id:interpreter==="agent"?value(f,"agent_id"):userID,uncertainty:value(f,"uncertainty"),dissent:lines(value(f,"dissent"))}})}}><Field name="segment" label="Segment"/><Area name="exposures" label="Variant exposure: key:count"/><Area name="effects" label="Segment effect: metric:value"/><Area name="segment_uncertainty" label="Metric uncertainty: metric:value"/><Area name="exclusions" label="Excluded evidence and why"/><Area name="guardrails" label="Guardrail outcomes"/><Area name="interpretation" label="Evidence-bound interpretation"/><Select name="interpreter_type" label="Interpreter" initial="human" options={["human","agent"]}/><Field name="agent_id" label="Approved agent ID (when applicable)" required={false}/><Area name="uncertainty" label="Remaining uncertainty"/><Area name="dissent" label="Attributed dissent"/><p className="text-sm text-[var(--muted)]">Human analysis is attributed to {userID}. Agent analysis requires this participant to be a current operator of the named organization-approved agent.</p><Button>Freeze analysis</Button></form>}
    {analysis&&<article className="mt-4 rounded-lg border p-4 text-sm"><div className="flex gap-2"><Badge>plan v{analysis.experiment_version}</Badge><Badge>{analysis.threshold_reason.replaceAll("_"," ")}</Badge><Badge>{analysis.interpreted_by_type} {analysis.interpreted_by_id}</Badge></div><p className="mt-2"><b>Interpretation:</b> {analysis.interpretation}</p><p><b>Uncertainty:</b> {analysis.uncertainty}</p><p><b>Guardrails:</b> {analysis.guardrail_outcomes.join(" · ")}</p><p><b>Exclusions:</b> {analysis.exclusions.join(" · ")||"None"}</p><p><b>Dissent:</b> {analysis.dissent.join(" · ")||"None recorded"}</p>{analysis.segment_effects.map(x=><p key={x.segment}><b>{x.segment}:</b> {Object.entries(x.metric_values).map(([k,v])=>`${k} ${v} ±${x.uncertainty[k]??"?"}`).join(" · ")}</p>)}</article>}
    {analysis&&!decision&&<form className="mt-4 grid gap-3 md:grid-cols-3" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget),choice=value(f,"decision");void act("outcomes",{expected_version:experiment.outcome_decisions?.length??0,decision:{analysis_id:analysis.id,decision:choice,variant_key:choice==="adopt_variant"?value(f,"variant_key"):"",rationale:value(f,"rationale"),tasks:taskKinds(choice).map(kind=>({kind,title:`${kind.replaceAll("_"," ")} for experiment ${experiment.id}`}))}})}}><Select name="decision" label="Product decision" initial="adopt_variant" options={["adopt_variant","retain_control","extend_test","inconclusive"]}/><Field name="variant_key" label="Adopted variant (when applicable)" initial="treatment"/><Area name="rationale" label="Justification from frozen evidence"/><Button>Record outcome and create tasks</Button></form>}
    {decision&&<div className="mt-4"><div className="flex gap-2"><Badge>{decision.decision.replaceAll("_"," ")}</Badge><Badge tone={decision.cleaned_up?"success":"warning"}>{decision.cleaned_up?"supported system clean":"cleanup open"}</Badge><Badge>decision v{decision.version}</Badge></div><p className="mt-2 text-sm">{decision.rationale}</p><div className="mt-3 grid gap-2 md:grid-cols-2">{decision.tasks.map(t=><form key={t.id} className="rounded-lg border p-3 text-sm" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);void act(`outcomes/${decision.id}/tasks/${t.id}/complete`,{expected_version:decision.version,evidence:{pull_request_id:value(f,"pull_request_id"),kind:value(f,"evidence_kind"),resource_id:value(f,"resource_id")}})}}><div><b>{t.title}</b> {t.required&&<Badge>required</Badge>}</div><p className="text-xs text-[var(--muted)]">{t.status}{t.evidence&&` · ${t.evidence.kind} ${t.evidence.resource_id}`}</p>{t.status!=="completed"&&<div className="mt-2 grid gap-2"><Select name="evidence_kind" label="Verified evidence" initial="pull_request" options={["pull_request","release","deployment"]}/><input className="min-h-10 rounded-lg border px-3" name="pull_request_id" required placeholder="Merged pull with exact outcome trailers"/><input className="min-h-10 rounded-lg border px-3" name="resource_id" required placeholder="Pull, release, or deployment ID"/><p className="text-xs text-[var(--muted)]">The merged pull body must name this experiment, decision, task, and action.</p><Button>Verify and complete</Button></div>}</form>)}</div></div>}
  </Card>
}
