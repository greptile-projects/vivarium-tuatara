...
type OutcomeDecision = { id:string; version:number; experiment_version:number; analysis_id:string; decision:string; variant_key?:string; rationale:string; cleaned_up:boolean; tasks:{id:string;kind:string;title:string;required:boolean;status:string;evidence_url?:string;completed_by?:string}[] };
type RunAttempt = {
...
function ExperimentOutcome({experiment,userID,act}:{experiment:Experiment;userID:string;act:(path:string,body:unknown)=>Promise<void>}) {
  ...
  return <Card className="p-5"><h2 className="font-semibold">Decision and supported-system cleanup</h2><p className="mt-1 text-sm text-[var(--muted)]">Once evidence or a stop condition is reached, freeze the analysis, retain dissent, choose an outcome, and prove every obsolete flag, rule, credential, and collection path is retired.</p>
    {run&&!analysis&&<form className="mt-4 grid gap-3 md:grid-cols-3" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);void act("analyses",{analysis:{run_id:run.id,run_version:run.version,segment_effects:[{segment:value(f,"segment"),exposures:pairs(value(f,"exposures")),metric_values:pairs(value(f,"effects")),uncertainty:pairs(value(f,"segment_uncertainty"))}],exclusions:lines(value(f,"exclusions")),guardrail_outcomes:lines(value(f,"guardrails")),interpretation:value(f,"interpretation"),interpreted_by_type:value(f,"interpreter_type"),interpreted_by_id:value(f,"interpreter_id"),uncertainty:value(f,"uncertainty"),dissent:lines(value(f,"dissent"))}})}}><Field name="segment" label="Segment"/><Area name="exposures" label="Variant exposure: key:count"/><Area name="effects" label="Segment effect: metric:value"/><Area name="segment_uncertainty" label="Metric uncertainty: metric:value"/><Area name="exclusions" label="Excluded evidence and why"/><Area name="guardrails" label="Guardrail outcomes"/><Area name="interpretation" label="Evidence-bound interpretation"/><Area name="uncertainty" label="Remaining uncertainty"/><Area name="dissent" label="Attributed dissent"/><Select name="interpreter_type" label="Interpreter" initial="human" options={["human","agent"]}/><Field name="interpreter_id" label="Interpreter identity" initial={userID}/><Button>Freeze analysis</Button></form>}
