            ...
            {runs.map((run) => {
              ...
              return <div key={run.id} className="p-4 sm:px-5">
                ...
                {open && <div className="mt-4 space-y-4 border-t border-[var(--line)] pt-4">
                  {run.definition.documentation && <div className="rounded-lg bg-[var(--surface-soft)] p-3 text-xs">
                    <div className="flex flex-wrap items-center gap-2"><Badge tone="info">documentation</Badge><strong>{run.definition.documentation.source} · {run.definition.documentation.version}</strong><span className="text-[var(--muted)]">target {short(run.definition.documentation.revision)}</span></div>
                    <p className="mt-2 text-[var(--muted)]">Coverage: {run.definition.documentation.selectors.join(", ")}</p>
                    <p className="mt-1 text-[var(--muted)]">Affected evidence: {run.definition.documentation.dependency_paths.join(", ")} · input digest <code>{short(run.definition.documentation.dependency_sha256)}</code></p>
                    <p className="mt-1 text-[var(--muted)]">Commands may publish retained builds, coverage, and expected/actual output differences through artifacts below.</p>
                  </div>}
