...
export function SecurityAdvisoriesWorkspace({ advisoryId }: { advisoryId?: string }) {
  ...
  const [investigationAccess, setInvestigationAccess] = useState<{ token: string; expires_at: string }>();
  ...
  const delegate = async (form: HTMLFormElement) => {
    ...
    try {
      const launched = await api<{ security_advisory: SecurityAdvisory; credential: { token: string; expires_at: string } }>(`/security-advisories/${advisory.id}/investigations`, { method: "POST", body: JSON.stringify({ mandate: data.get("mandate"), evidence_ids: data.getAll("evidence_id"), expires_in: 3600 }) }, token);
      setAdvisory(launched.security_advisory); setInvestigationAccess(launched.credential); form.reset();
  ...
  if (advisoryId && advisory) return <div className="space-y-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
      ...
      <Card className="p-5"><h2 className="font-semibold">Delegate selected evidence</h2><p className="mt-2 text-sm text-[var(--muted)]">A short-lived read-only agent sees only the frozen evidence you select. Its findings and uncertainty return only to this protected workspace.</p>{investigationAccess && <div role="status" className="mt-4 rounded-lg border border-[var(--warning)] bg-[var(--warning-soft)] p-3"><p className="text-sm font-semibold">Copy this agent credential now</p><p className="mt-1 text-xs text-[var(--muted)]">It is shown only in this browser session and expires {stamp(investigationAccess.expires_at)}.</p><input className={`${field} font-mono text-xs`} readOnly aria-label="Investigation agent credential" value={investigationAccess.token}/><div className="mt-2 flex gap-2"><Button type="button" onClick={()=>void navigator.clipboard.writeText(investigationAccess.token)}>Copy credential</Button><Button type="button" variant="secondary" onClick={()=>setInvestigationAccess(undefined)}>Clear</Button></div></div>}<form className="mt-4" onSubmit={(event)=>{event.preventDefault();void delegate(event.currentTarget);}}><div className="space-y-2">{advisory.evidence.filter((x)=>x.id).map((x)=><label key={x.id} className="flex gap-2 text-sm"><input type="checkbox" name="evidence_id" value={x.id}/>{x.label}</label>)}</div><textarea className={`${field} py-3`} name="mandate" required rows={3} maxLength={10000} placeholder="Investigation mandate"/><Button className="mt-3" disabled={pending || !advisory.evidence.some((x)=>x.id)}>Delegate read-only investigation</Button></form>{advisory.investigations.map((x)=><div key={x.id} className="mt-3 rounded-lg bg-[var(--canvas)] p-3 text-sm"><Badge>{x.state}</Badge><p className="mt-2">{x.mandate}</p><p className="mt-1 text-xs text-[var(--muted)]">{x.evidence.length} frozen evidence item(s)</p></div>)}</Card>
