...
type Contract = {
  ...
  improvements: {id:string;objective_id:string;investigation_id?:string;finding_id?:string;impact_id?:string;baseline_observation_ids:string[];affected_observation_ids:string[];affected_revisions:string[];dependency_context:string[];evidence_ids:string[];acceptance_criteria:string[];proposal_id:string;task_ids:string[];status:string;created_by:string}[];
  rollout_verifications: {id:string;improvement_id:string;kind:string;resource_id:string;revision:string;baseline_observation_id:string;current_observation_id:string;decision:string;rationale:string;budget_before_percent?:number;budget_after_percent?:number;improved:boolean;budget_restored:boolean;recorded_by:string}[];
};
...
export function ServiceObjectivesWorkspace({
  ...
  async function createImprovement(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!token||!selected||!current)return;const f=new FormData(e.currentTarget);try{const out=await api<Contract>(`/repositories/${repositoryID}/service-objectives/${selected.id}/improvements`,{method:"POST",body:JSON.stringify({contract_version:current.version,objective_id:value(f,"improvement_objective"),investigation_id:value(f,"improvement_investigation")||undefined,finding_id:value(f,"improvement_finding")||undefined,impact_id:value(f,"improvement_impact")||undefined,title:value(f,"improvement_title"),body:value(f,"improvement_body"),baseline_observation_ids:list(value(f,"improvement_baselines")),affected_observation_ids:list(value(f,"improvement_affected")),affected_revisions:list(value(f,"improvement_revisions")),dependency_context:list(value(f,"improvement_dependencies")),evidence_ids:list(value(f,"improvement_evidence")),acceptance_criteria:list(value(f,"improvement_criteria")),tasks:value(f,"improvement_tasks").split(";").filter(Boolean).map((x,i)=>{const [title,assignee_type,assignee_id]=x.split("|").map(y=>y.trim());return {title,assignee_type,assignee_id,depends_on_previous:i>0}})})},token);setSelected(out);setItems(xs=>[out,...xs.filter(x=>x.id!==out.id)]);setError("")}catch(x){setError(x instanceof Error?x.message:"Reliability improvement could not be created.")}}
  async function verifyImprovement(e:FormEvent<HTMLFormElement>, improvement:Contract["improvements"][number]){e.preventDefault();if(!token||!selected)return;const f=new FormData(e.currentTarget);try{const out=await api<Contract>(`/repositories/${repositoryID}/service-objectives/${selected.id}/improvements/${improvement.id}/verifications`,{method:"POST",body:JSON.stringify({verification:{kind:value(f,"verify_kind"),resource_id:value(f,"verify_resource"),revision:value(f,"verify_revision"),baseline_observation_id:value(f,"verify_baseline"),current_observation_id:value(f,"verify_current"),decision:value(f,"verify_decision"),rationale:value(f,"verify_rationale")}})},token);setSelected(out);setItems(xs=>[out,...xs.filter(x=>x.id!==out.id)]);setError("")}catch(x){setError(x instanceof Error?x.message:"Rollout verification could not be recorded.")}}
  return (
    <div className="space-y-8">
      ...
      {selected && current && <Card className="p-5"><h2 className="font-semibold">Deliver and verify an improvement</h2><p className="mt-1 text-sm text-[var(--muted)]">Convert a finding or depleted budget into ordinary ordered work, then compare governed release or deployment signals with the retained baseline.</p><form onSubmit={createImprovement} className="mt-4 grid gap-3 md:grid-cols-2"><Field n="improvement_title" l="Proposal title"/><Area n="improvement_body" l="Objective and user harm"/><Field n="improvement_objective" l="Objective ID" v={selectedObjective?.id}/><Field n="improvement_investigation" l="Investigation ID (finding source)" required={false}/><Field n="improvement_finding" l="Finding ID (or use impact)" required={false}/><Field n="improvement_impact" l="Depleted-budget impact ID (or use finding)" required={false}/><Field n="improvement_baselines" l="Baseline observation IDs"/><Field n="improvement_affected" l="Affected observation IDs"/><Field n="improvement_revisions" l="Affected revisions"/><Field n="improvement_dependencies" l="Dependency context"/><Field n="improvement_evidence" l="Evidence IDs"/><Field n="improvement_criteria" l="Acceptance criteria"/><div className="md:col-span-2"><Area n="improvement_tasks" l="Ordered tasks: title | human/agent | assignee ID (semicolon separated)"/><Button>Create accountable work</Button></div></form>{(selected.improvements??[]).map(x=><div key={x.id} className="mt-5 border-t pt-4 text-sm"><div className="flex flex-wrap gap-2"><strong>Objective {x.objective_id}</strong><Badge>{x.status}</Badge>{x.status==="linked"&&<><Badge>{x.task_ids.length} ordered tasks</Badge><Badge>proposal {x.proposal_id}</Badge></>}</div><p className="mt-2">Acceptance: {x.acceptance_criteria.join(" · ")}</p><p className="mt-1 text-xs text-[var(--muted)]">Affected {x.affected_revisions.join(", ")} · dependencies {x.dependency_context.join(", ")||"none"} · authored by {x.created_by}</p>{x.status==="pending"?<p className="mt-3 text-xs text-[var(--warning)]">Proposal publication is pending recovery; retry the exact plan to finish its link.</p>:<form onSubmit={e=>verifyImprovement(e,x)} className="mt-3 grid gap-2 md:grid-cols-3"><Select n="verify_kind" l="Rollout resource" v="release" options={["release","deployment"]}/><Field n="verify_resource" l="Resource ID"/><Field n="verify_revision" l="Exact revision"/><Field n="verify_baseline" l="Baseline observation ID" v={x.baseline_observation_ids[0]}/><Field n="verify_current" l="Current observation ID"/><Select n="verify_decision" l="Decision" v="restore_budget" options={["restore_budget","contain","rollback","revisit"]}/><div className="md:col-span-3"><Area n="verify_rationale" l="Comparison and decision rationale"/><Button>Record governed verification</Button></div></form>}{(selected.rollout_verifications??[]).filter(v=>v.improvement_id===x.id).map(v=><p key={v.id} className="mt-3 rounded border p-3"><strong>{v.decision.replaceAll("_"," ")}</strong> · budget {v.budget_before_percent?.toFixed(1)}% → {v.budget_after_percent?.toFixed(1)}% · {v.improved?"measured improvement":"measure failed"}{v.budget_restored?" · budget restored with history retained":""}<br/><span className="text-xs text-[var(--muted)]">{v.rationale} — {v.recorded_by}</span></p>)}</div>)}</Card>}
