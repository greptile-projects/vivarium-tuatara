...
type Evaluation = { id:string; revision:string; goal_id:string; investigation_id:string; baseline_trial_id:string; candidate_trial_id:string; affected_scenarios:string[]; commands:string[]; correctness_checks:{name:string;command:string;passed:boolean;summary:string}[]; residual_risks:string[]; created_by:string; confidence:number|null; comparisons:{metric:string;unit:string;baseline_mean:number;current_mean:number;change_percent:number;comparable:boolean;reason?:string}[]; resource_changes:Record<string,number>; cost_change_percent:number|null; correctness_passed:boolean; stale:boolean };
...
export function PullPerformanceEvaluations({repositoryID,pullRequestID}:{repositoryID:string;pullRequestID:string}) {
  ...
  return <section id="performance" className="scroll-mt-24 space-y-4" aria-label="Performance evaluations">
    ...
    {items.map(item=><Card className="p-5" key={item.id}>
      ...
      <p className="mt-4 text-sm"><strong>Resources:</strong> CPU {item.resource_changes.cpu_seconds_percent.toFixed(1)}%, memory {item.resource_changes.peak_memory_mb_percent.toFixed(1)}%, cost {item.cost_change_percent === null ? "unavailable" : `${item.cost_change_percent.toFixed(1)}%`}</p>
