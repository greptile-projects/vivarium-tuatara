        ...
        <Card key={item.id} className="p-5">
          ...
          {workspaceID && (
            <>
              ...
              {item.conflict_context && <ConflictMeaning workspace={item} onWorkspace={(updated) => setItems([updated])} />}
...
type ComparisonSide={revision:string;content:string;sha256:string;missing:boolean};
type Comparison={path:string;base:ComparisonSide;source:ComparisonSide;target:ComparisonSide;proposed:ComparisonSide};
function ConflictMeaning({workspace,onWorkspace}:{workspace:DevelopmentWorkspace;onWorkspace:(workspace:DevelopmentWorkspace)=>void}) {
  const {token}=useAuth(), context=workspace.conflict_context!, [comparison,setComparison]=useState<Comparison>(), [pending,setPending]=useState(false), [error,setError]=useState("");
  const [path,setPath]=useState(context.files[0]?.path||"");
  async function compare(next=path){if(!token||!next)return;setPending(true);try{setComparison(await api<Comparison>(`/workspaces/${workspace.id}/conflict-comparison?path=${encodeURIComponent(next)}`,{},token));setError("")}catch(e){setError(e instanceof APIError?e.message:"Comparison could not be loaded.")}finally{setPending(false)}}
  useEffect(()=>{if(!token||!path)return;let active=true;void api<Comparison>(`/workspaces/${workspace.id}/conflict-comparison?path=${encodeURIComponent(path)}`,{},token).then(value=>{if(active)setComparison(value)}).catch(reason=>{if(active)setError(reason instanceof APIError?reason.message:"Comparison could not be loaded.")});return()=>{active=false}},[token,workspace.id,path]);
  const citation=(side:"base"|"source"|"target"|"proposed")=>({side,revision:side==="proposed"?`workspace:${workspace.id}`:side==="base"?context.base_commit_id:context[side].commit_id,path});
  async function question(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!token)return;const form=event.currentTarget,data=new FormData(form),side=String(data.get("side")) as "base"|"source"|"target"|"proposed";setPending(true);try{onWorkspace(await api<DevelopmentWorkspace>(`/workspaces/${workspace.id}/conflict-questions`,{method:"POST",body:JSON.stringify({expected_version:context.version,body:String(data.get("body")),uncertainty:String(data.get("uncertainty")),citations:[citation(side)]})},token));form.reset()}catch(e){setError(e instanceof APIError?e.message:"Question could not be retained.")}finally{setPending(false)}}
  async function answer(event:FormEvent<HTMLFormElement>,id:string){event.preventDefault();if(!token)return;const form=event.currentTarget,data=new FormData(form),side=String(data.get("side")) as "base"|"source"|"target"|"proposed";setPending(true);try{onWorkspace(await api<DevelopmentWorkspace>(`/workspaces/${workspace.id}/conflict-questions/${id}/answer`,{method:"POST",body:JSON.stringify({expected_version:context.version,body:String(data.get("body")),uncertainty:String(data.get("uncertainty")),citations:[citation(side)]})},token));form.reset()}catch(e){setError(e instanceof APIError?e.message:"Answer could not be retained.")}finally{setPending(false)}}
  async function propose(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!token||!comparison)return;const form=event.currentTarget,data=new FormData(form),side=String(data.get("side")) as "base"|"source"|"target"|"proposed";setPending(true);try{onWorkspace(await api<DevelopmentWorkspace>(`/workspaces/${workspace.id}/conflict-resolutions`,{method:"POST",body:JSON.stringify({expected_version:context.version,path,summary:String(data.get("summary")),proposed_content:String(data.get("content")),expected_sha256:comparison.proposed.sha256,uncertainty:String(data.get("uncertainty")),preservation:[{kind:String(data.get("kind")),reference:String(data.get("reference")),disposition:String(data.get("disposition")),rationale:String(data.get("rationale")),citations:[citation(side)]}]})},token));form.reset()}catch(e){setError(e instanceof APIError?e.message:"Resolution could not be proposed.")}finally{setPending(false)}}
  async function act(id:string,action:"apply"|"undo"){if(!token)return;setPending(true);try{const updated=await api<DevelopmentWorkspace>(`/workspaces/${workspace.id}/conflict-resolutions/${id}/${action}`,{method:"POST",body:JSON.stringify({expected_version:context.version})},token);onWorkspace(updated);await compare()}catch(e){setError(e instanceof APIError?e.message:"Resolution action failed.")}finally{setPending(false)}}
  const field="mt-1 w-full rounded-lg border border-[var(--line)] bg-white p-2 font-normal";
  return <section className="mt-5 rounded-xl border border-[var(--line)] p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand)]">Meaning ledger · v{context.version}</p><h3 className="mt-1 font-semibold">Compare intent, ask, and resolve</h3><p className="mt-1 text-xs text-[var(--muted)]">Every claim cites frozen evidence. Applying changes requires the visible file-control lease and grants no merge, secret, or environment authority.</p></div><label className="text-xs font-semibold">Overlapping file<select className={field} value={path} onChange={e=>setPath(e.target.value)}>{context.files.map(file=><option key={file.path}>{file.path}</option>)}</select></label></div>{comparison&&<div className="mt-4 grid gap-2 lg:grid-cols-4">{(["base","source","target","proposed"] as const).map(side=><div key={side} className="min-w-0 rounded-lg bg-[var(--canvas)] p-3"><p className="text-xs font-semibold uppercase">{side} · {comparison[side].revision.slice(0,12)}</p><pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap text-xs">{comparison[side].missing?"File absent":comparison[side].content}</pre></div>)}</div>}<div className="mt-5 grid gap-4 lg:grid-cols-2"><form onSubmit={question} className="rounded-lg border border-[var(--line)] p-3"><h4 className="font-semibold">Ask a revision-grounded question</h4><textarea name="body" required className={field} placeholder="Which behavior is intentional here?"/><input name="uncertainty" required className={field} placeholder="What remains uncertain?"/><select name="side" className={field}><option>source</option><option>target</option><option>base</option><option>proposed</option></select><Button className="mt-2" disabled={pending}>Retain question</Button></form><form onSubmit={propose} className="rounded-lg border border-[var(--line)] p-3"><h4 className="font-semibold">Propose a bounded resolution</h4><input name="summary" required className={field} placeholder="How this combines both outcomes"/><textarea name="content" required defaultValue={comparison?.proposed.content} className={`${field} min-h-32 font-mono text-xs`}/><div className="grid grid-cols-2 gap-2"><select name="kind" className={field}><option value="acceptance_criterion">Acceptance criterion</option><option value="design_decision">Design decision</option><option value="migration">Migration</option><option value="user_behavior">User behavior</option></select><select name="disposition" className={field}><option value="preserved">Preserved</option><option value="intentionally_changed">Intentionally changed</option></select></div><input name="reference" required className={field} placeholder="Named criterion or decision"/><input name="rationale" required className={field} placeholder="Why this edit preserves or changes it"/><input name="uncertainty" required className={field} placeholder="Residual uncertainty"/><select name="side" className={field}><option>source</option><option>target</option><option>base</option><option>proposed</option></select><Button className="mt-2" disabled={pending||!comparison?.proposed.sha256}>Propose without applying</Button></form></div><div className="mt-5 space-y-3">{context.questions.map(q=><article key={q.id} className="rounded-lg bg-[var(--canvas)] p-3 text-sm"><p><b>Question:</b> {q.body}</p><p className="mt-1 text-xs text-[var(--muted)]">{q.authorship.agent_id?`Agent ${short(q.authorship.agent_id)} operated by ${short(q.authorship.actor_id)}`:`Human ${short(q.authorship.actor_id)}`} · uncertainty: {q.uncertainty}</p>{q.answer?<p className="mt-2"><b>Answer:</b> {q.answer.body} <span className="text-xs text-[var(--muted)]">({q.answer.uncertainty})</span></p>:<form className="mt-2 grid gap-2 sm:grid-cols-4" onSubmit={e=>void answer(e,q.id)}><input name="body" required className={field} placeholder="Grounded answer"/><input name="uncertainty" required className={field} placeholder="Remaining uncertainty"/><select name="side" className={field}><option>source</option><option>target</option><option>base</option><option>proposed</option></select><Button disabled={pending}>Answer</Button></form>}</article>)}{context.resolutions.map(r=><article key={r.id} className="rounded-lg border border-[var(--line)] p-3 text-sm"><div className="flex justify-between gap-3"><div><Badge tone={r.state==="applied"?"success":r.state==="undone"?"warning":"info"}>{r.state}</Badge><b className="ml-2">{r.summary}</b><p className="mt-2 text-xs">{r.preservation.map(x=>`${x.kind.replaceAll("_"," ")}: ${x.reference} — ${x.disposition}`).join("; ")}</p><p className="mt-1 text-xs text-[var(--muted)]">Uncertainty: {r.uncertainty} · {r.authorship.agent_id?`agent ${short(r.authorship.agent_id)} / operator ${short(r.authorship.actor_id)}`:`human ${short(r.authorship.actor_id)}`}</p></div>{r.state!=="undone"&&<Button disabled={pending} onClick={()=>void act(r.id,r.state==="proposed"?"apply":"undo")}>{r.state==="proposed"?"Apply with control":"Undo edit"}</Button>}</div></article>)}</div>{error&&<p role="alert" className="mt-3 text-sm text-[var(--danger)]">{error}</p>}</section>
}
