...
export function ChangeSessionDetail({ repositoryID, pullRequestID, sessionID }: { repositoryID: string; pullRequestID: string; sessionID: string }) {
  ...
  const repair = session.check_evidence;
  const deployment = session.deployment_evidence;
  const preview = session.preview_evidence;
  return <div className="space-y-7">
    ...
    {preview && <Card className="border-[var(--brand)] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-[var(--brand)]">Preview finding repair</p><h2 className="mt-1 font-semibold">{preview.title}</h2></div><Badge tone={preview.severity === "blocking" ? "danger" : "warning"}>{preview.classification} · {preview.severity}</Badge></div><p className="mt-3 text-sm leading-6">{preview.description}</p><p className="mt-3 text-xs text-[var(--muted)]"><code>{preview.route}</code> · observed by {short(preview.author_id)} · revision <code>{short(preview.revision)}</code></p><h3 className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Acceptance criteria</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm">{preview.acceptance_criteria.map(criterion=><li key={criterion}>{criterion}</li>)}</ul>{preview.reproduction_steps.length > 0 && <><h3 className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Reproduction</h3><ol className="mt-2 list-decimal space-y-1 pl-5 text-sm">{preview.reproduction_steps.map((step,index)=><li key={`${index}-${step}`}>{step}</li>)}</ol></>}{preview.evidence.length > 0 && <p className="mt-4 text-xs text-[var(--muted)]">Permitted evidence: {preview.evidence.map(item=>`${item.kind} ${item.name}${item.redacted ? " (redacted)" : ""}`).join(", ")}</p>}{preview.discussion.length > 0 && <div className="mt-4 space-y-2">{preview.discussion.map(item=><p key={item.id} className="rounded-lg bg-[var(--surface-subtle)] p-3 text-sm"><span className="font-semibold">{short(item.author_id)}</span> {item.body}</p>)}</div>}<p className="mt-4 text-sm text-[var(--muted)]">This context grants no additional authority. Human edits and agent runs remain limited to the pull source branch and ordinary review.</p></Card>}
    {repair && <Card className="border-[var(--danger)] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-[var(--danger)]">Automated failure</p><h2 className="mt-1 font-semibold">{repair.definition.name}</h2></div><Badge tone="danger">failed · {short(session.source_commit_id)}</Badge></div><p className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Versioned command · {repair.definition.image} · {repair.definition.working_directory || "."}</p><pre className="mt-2 overflow-auto rounded-lg bg-[var(--ink)] p-4 text-xs text-white">{repair.definition.command}</pre><p className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Relevant logs</p><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap rounded-lg bg-[var(--ink)] p-4 text-xs leading-5 text-white">{repair.events.filter((event) => event.kind === "log").map((event) => `[attempt ${event.attempt} · ${event.stream}] ${event.message || ""}`).join("") || "No log output recorded."}</pre><p className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Artifacts</p>{repair.artifacts.length ? <ul className="mt-2 space-y-1">{repair.artifacts.map((artifact) => <li key={artifact.id} className="break-all font-mono text-xs">{artifact.path} · {artifact.size.toLocaleString()} bytes · sha256 {artifact.sha256}</li>)}</ul> : <p className="mt-2 text-sm text-[var(--muted)]">No artifacts were published.</p>}</Card>}
    {deployment && <Card className="border-[var(--danger)] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[.14em] text-[var(--danger)]">Unhealthy deployment</p><h2 className="mt-1 font-semibold">Release {deployment.release_version} · environment {short(deployment.environment_id)}</h2></div><Badge tone="danger">{deployment.state} · stage {deployment.current_stage + 1}</Badge></div><p className="mt-3 whitespace-pre-wrap text-sm leading-6">{deployment.release_notes}</p><dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="font-semibold text-[var(--muted)]">Source revision</dt><dd className="mt-1 font-mono">{deployment.commit_id}</dd></div><div><dt className="font-semibold text-[var(--muted)]">Artifact</dt><dd className="mt-1 break-all font-mono">{deployment.artifact_id} · sha256:{deployment.artifact_sha256}</dd></div></dl><p className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Health evidence</p>{deployment.evidence.length ? <ul className="mt-2 space-y-2">{deployment.evidence.map((item, index) => <li key={`${item.stage}-${item.signal}-${index}`} className="text-xs"><Badge tone={item.state === "passed" ? "success" : "danger"}>{item.state}</Badge><span className="ml-2 font-semibold">{item.stage} / {item.signal}</span>{item.message ? ` — ${item.message}` : ""}</li>)}</ul> : <p className="mt-2 text-sm text-[var(--muted)]">No health signal completed before failure.</p>}<p className="mt-4 text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Deployment logs and decisions</p><ol className="mt-2 max-h-72 space-y-1 overflow-auto rounded-lg bg-[var(--ink)] p-4 text-xs text-white">{deployment.events.map((event) => <li key={event.sequence}>{event.sequence}. {event.kind}{event.state ? ` [${event.state}]` : ""}{event.message ? ` — ${event.message}` : ""}</li>)}</ol><p className="mt-4 text-sm leading-6 text-[var(--muted)]">This workspace can only write its repair branch. Any fix must pass pull request review, required checks, integration policy, and a newly built release before it can reach an environment.</p></Card>}
