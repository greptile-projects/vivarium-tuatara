...
import { api, type LearningAssessment, type LearningAssessmentAttempt, type LearningOutcomeReport, type LearningPathway, type Repository } from "@/lib/api";
...
function Pathway({
  ...
  return (
    <main className="space-y-5">
      ...
      <OutcomePanel pathway={p} repositoryId={repositoryId} token={token} onError={onError} />
    </main>
  );
}
...
function OutcomePanel({ pathway, repositoryId, token, onError }: { pathway: LearningPathway; repositoryId: string; token: string | null; onError: (v: string) => void }) {
  const [report,setReport]=useState<LearningOutcomeReport|null>(null);
  const [form,setForm]=useState({kind:"module_completion",state:"completed",module_id:pathway.modules[0]?.id??"",detail:"",visibility:"private"});
  const load=useCallback(async()=>{try{setReport(await api<LearningOutcomeReport>(`/repositories/${repositoryId}/learning-pathways/${pathway.slug}/outcomes`,{},token))}catch(e){onError(e instanceof Error?e.message:"Could not load learning outcomes")}},[repositoryId,pathway.slug,token,onError]);
  useEffect(()=>{void Promise.resolve().then(load)},[load]);
  async function submit(){try{await api(`/repositories/${repositoryId}/learning-pathways/${pathway.slug}/outcomes`,{method:"POST",body:JSON.stringify({...form,pathway_version:pathway.version,request_id:crypto.randomUUID(),consent:true})},token);onError("");await load()}catch(e){onError(e instanceof Error?e.message:"Could not retain learning outcome")}}
  const labels:Record<string,string>={module_completion:"Module completion",recurring_question:"Recurring questions",setup_failure:"Setup failures",assessment_gap:"Assessment gaps",mentor_load:"Mentor load",contribution_outcome:"Contribution outcomes",reviewer_correction:"Reviewer corrections",retention:"Retention"};
  return <section className="rounded-xl border border-[var(--line)] bg-white p-6"><p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand-strong)]">Pathway improvement</p><h3 className="mt-1 text-xl font-semibold">Outcomes, with learner control</h3><p className="mt-1 text-sm text-[var(--muted)]">Nothing is inferred from browsing. You choose what to report and whether it stays private, goes to maintainers, or joins a privacy-thresholded aggregate.</p>
    {report&&<div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(labels).map(([key,label])=><div key={key} className="rounded-lg bg-[var(--canvas)] p-3"><p className="text-xs text-[var(--muted)]">{label}</p><p className="mt-1 text-lg font-semibold">{report.aggregates[key]??"—"}</p></div>)}</div>}
    {report&&report.stale_completion_evidence>0&&<p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">{report.stale_completion_evidence} completion record(s) belong to archived pathway revisions. Those achievements remain valid historical records; changed material requirements require explicit revalidation.</p>}
    {token&&<div className="mt-5 grid gap-3 md:grid-cols-2"><label className="text-xs font-semibold text-[var(--muted)]">Outcome category<select value={form.kind} onChange={e=>setForm({...form,kind:e.target.value})} className="mt-1 w-full rounded-lg border border-[var(--line)] p-2 text-sm text-[var(--ink)]">{Object.entries(labels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label className="text-xs font-semibold text-[var(--muted)]">Visibility<select value={form.visibility} onChange={e=>setForm({...form,visibility:e.target.value})} className="mt-1 w-full rounded-lg border border-[var(--line)] p-2 text-sm text-[var(--ink)]"><option value="private">Only me</option><option value="maintainers">Maintainers</option><option value="aggregate">Anonymous aggregate</option></select></label><label className="text-xs font-semibold text-[var(--muted)]">Outcome state<input value={form.state} onChange={e=>setForm({...form,state:e.target.value})} className="mt-1 w-full rounded-lg border border-[var(--line)] p-2 text-sm text-[var(--ink)]" /></label><label className="text-xs font-semibold text-[var(--muted)]">Optional context<textarea value={form.detail} onChange={e=>setForm({...form,detail:e.target.value})} className="mt-1 w-full rounded-lg border border-[var(--line)] p-2 text-sm text-[var(--ink)]" /></label><button onClick={()=>void submit()} className="rounded-lg bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white">Consent and report outcome</button></div>}
    <p className="mt-4 text-xs text-[var(--muted)]">Aggregates appear only with at least {report?.suppression_threshold??3} distinct consenting learners. Only each learner&apos;s latest state counts, and private reports never support maintainer findings.</p></section>;
}
