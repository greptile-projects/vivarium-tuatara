...
export function DecisionsWorkspace({ decisionId }: { decisionId?: string }) {
  ...
  async function requestApproval(e: FormEvent<HTMLFormElement>) {
    ...
    try { setCurrent(await api<TechnicalDecision>(`/decisions/${current.id}/approval-requests`,{method:"POST",body:JSON.stringify({expected_version:current.version,request:{kind,repository_id:kind==="affected_owner"?d.get("repository_id"):undefined,policy_id:kind==="policy"?d.get("policy_id"):undefined,policy_rule:kind==="policy"?d.get("policy_rule"):undefined,approver_id:d.get("approver_id"),reason:d.get("reason")}})},token)); form.reset(); } catch(x){setError(x instanceof Error?x.message:"Approval could not be requested.")} finally{setPending(false)}
  }
