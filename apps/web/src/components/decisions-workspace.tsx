...
export function DecisionsWorkspace({ decisionId }: { decisionId?: string }) {
  ...
  async function createImplementation(e: FormEvent<HTMLFormElement>) {
    e.preventDefault(); if (!current) return; setPending(true); setError(""); const form=e.currentTarget,d=new FormData(form);
    const tasks=lines(d.get("tasks")).map((row,index)=>{const [title,assignee_type,assignee_id,constraints,measures]=row.split("|").map(x=>x.trim());return{title,assignee_type,assignee_id:assignee_id||undefined,constraint_indexes:constraints.split(",").map(Number),success_measure_indexes:measures.split(",").map(Number),depends_on_previous:index>0}});
    const commitment=current.commitments.find(x=>x.status==="published"&&!current.implementations?.some(y=>y.commitment_version===x.version));
    try { const result=await api<{decision:TechnicalDecision;proposal:{id:string}}>(`/decisions/${current.id}/implementation`,{method:"POST",body:JSON.stringify({commitment_version:commitment?.version,title:d.get("title"),body:d.get("body"),tasks})},token); setCurrent(result.decision); form.reset(); router.push(`/proposals/${current.repository_id}/${result.proposal.id}`); }
    catch(x){setError(x instanceof Error?x.message:"Implementation plan could not be created.")}finally{setPending(false)}
  }
  async function reportDelivery(e:FormEvent<HTMLFormElement>,proposalID:string){e.preventDefault();if(!current)return;setPending(true);setError("");const form=e.currentTarget,d=new FormData(form);try{setCurrent(await api<TechnicalDecision>(`/decisions/${current.id}/implementation/${proposalID}/observations`,{method:"POST",body:JSON.stringify({kind:d.get("kind"),summary:d.get("summary"),resource_kind:d.get("resource_kind"),resource_id:d.get("resource_id")})},token));form.reset()}catch(x){setError(x instanceof Error?x.message:"Delivery evidence could not be recorded.")}finally{setPending(false)}}
  ...
  if (decisionId && current)
    return (
      <div className="space-y-6">
        ...
        <Card className="p-6">
          <h2 className="text-lg font-semibold">Accountable delivery</h2>
          <p className="mt-1 text-sm text-[var(--muted)]">Turn the accepted rationale into ordinary ordered work. The server derives completion criteria from every constraint and success measure; sessions, workspaces, pulls, checks, releases, and deployments retain the proposal and task links.</p>
          {current.implementations?.map(item=><article key={item.commitment_version} className="mt-4 rounded-lg bg-[var(--surface-2)] p-4 text-sm"><div className="flex gap-2"><Badge>Decision v{item.commitment_version}</Badge><Badge>{item.task_ids.length} accountable tasks</Badge></div><Link href={`/proposals/${current.repository_id}/${item.proposal_id}`} className="mt-2 inline-block font-semibold text-[var(--brand)]">Open delivery plan →</Link><p className="mt-1 text-xs text-[var(--muted)]">Frozen at {item.revision.slice(0,12)} · deviations require an explicit revisit</p>{item.observations?.map(x=><p key={x.id} className={`mt-2 rounded p-2 ${x.kind==="coverage"?"bg-[var(--success-soft)]":"bg-[var(--warning-soft)]"}`}><b>{label(x.kind)}:</b> {x.summary} · {x.resource_kind} {x.resource_id}</p>)}<form onSubmit={e=>reportDelivery(e,item.proposal_id)} className="mt-3 grid gap-2 sm:grid-cols-2"><label className="font-semibold">Outcome<select name="kind" className="mt-1 min-h-10 w-full rounded-lg border px-3 font-normal"><option value="coverage">Coverage confirmed</option><option value="deviation">Deviation</option><option value="assumption_changed">Assumption changed</option><option value="failed_measure">Success measure failed</option><option value="incompatible_work">Incompatible work</option></select></label><Field name="summary" title="Finding"/><Field name="resource_kind" title="Evidence stage (review, check, release, deployment)"/><Field name="resource_id" title="Linked resource ID"/><div><Button type="submit" disabled={pending}>Record delivery evidence</Button></div></form></article>)}
          {current.status==="published"&&current.scope.owner_id===user.id&&!current.implementations?.some(item=>current.commitments.some(c=>c.version===item.commitment_version&&c.status==="published"))&&<form onSubmit={createImplementation} className="mt-5 grid gap-3"><Field name="title" title="Implementation proposal title"/><label className="text-sm font-semibold">Delivery context<textarea name="body" required rows={3} className="mt-1 w-full rounded-lg border p-2 font-normal"/></label><label className="text-sm font-semibold">Ordered owned tasks<textarea name="tasks" required rows={5} placeholder="Title | human or agent | assignee ID (blank allocates agent) | constraint indexes | success measure indexes" className="mt-1 w-full rounded-lg border p-2 font-mono font-normal"/></label><p className="text-xs text-[var(--muted)]">Constraint indexes: {current.scope.constraints.map((x,i)=>`${i}: ${x}`).join(" · ")}<br/>Measure indexes: {current.scope.success_measures.map((x,i)=>`${i}: ${x}`).join(" · ")}</p><div><Button type="submit" disabled={pending}>Create accountable delivery plan</Button></div></form>}
        </Card>
        <Card className="p-6">
