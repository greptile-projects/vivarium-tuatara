...
export function ContributorPathwayWorkspace({ repositoryId }: { repositoryId: string }) {
  ...
  return <div className="space-y-6">
    ...
    <ContributionOutcomes repositoryId={repositoryId} token={token} />
...
function ContributionOutcomes({repositoryId,token}:{repositoryId:string;token:string|null}) {
  const [items,setItems]=useState<import("@/lib/api").ContributionOpportunity[]>([]);
  useEffect(()=>{let active=true;void api<{opportunities:import("@/lib/api").ContributionOpportunity[]}>(`/repositories/${repositoryId}/contribution-opportunities`,{},token).then(v=>{if(active)setItems(v.opportunities.filter(x=>x.completion))}).catch(()=>{});return()=>{active=false}},[repositoryId,token]);
  if(!items.length)return null;
  return <section><h2 className="text-xl font-semibold">Trusted contributor outcomes</h2><p className="mt-1 text-sm text-[var(--muted)]">Delivered work keeps its credit, support, feedback, and next-step readiness attached.</p><div className="mt-4 space-y-3">{items.map(item=>{const outcome=item.completion!;return <Card key={item.id} className="p-5"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{item.title}</h3><Badge tone="success">released {outcome.release_version}</Badge>{outcome.readiness.ready_for_next&&<Badge tone="info">ready for next contribution</Badge>}</div><p className="mt-3 text-sm">{outcome.feedback}</p><p className="mt-2 text-xs text-[var(--muted)]">Credit: {outcome.credit.join(", ")} · support: {outcome.support_effort.mentor_guidance_items} mentor, {outcome.support_effort.agent_assistance_items} agent, {outcome.support_effort.setup_attempts} setup/verification item(s)</p><p className="mt-2 text-sm"><strong>Next step:</strong> {outcome.readiness.note}</p>{outcome.readiness.skills_recognized.length>0&&<p className="mt-1 text-xs text-[var(--muted)]">Recognized skills: {outcome.readiness.skills_recognized.join(", ")}</p>}<p className="mt-3 text-xs text-[var(--muted)]">Contributor <code>{outcome.contributor_id.slice(0,8)}</code> · pull <code>{outcome.pull_request_id.slice(0,8)}</code> · recorded {new Date(outcome.recorded_at).toLocaleString()}</p></Card>})}</div></section>
}
