...
import { api, type ContributorPathway, type ContributorPathwayResponse, type Repository } from "@/lib/api";
...
export function ContributorPathwayWorkspace({ repositoryId }: { repositoryId: string }) {
  ...
  return <div className="space-y-6">
    ...
    {data && <Card className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-semibold">Revision {data.pathway.version}</p><p className="text-sm text-[var(--muted)]">Published {new Date(data.pathway.published_at).toLocaleString()} · {data.acknowledgements.filter((v) => v.version === data.pathway.version).length} acknowledgement(s)</p></div>{token && !acknowledged && <Button disabled={pending} onClick={acknowledge}>I understand these expectations</Button>}{acknowledged && <Badge tone="success">Acknowledged</Badge>}</Card>}
