...
export function CodeNavigationWorkspace({ id }: { id: string }) {
  ...
  return <div className="space-y-6">
    ...
    {result&&<><Card className="p-5"><div className="flex flex-wrap items-center gap-2"><Badge tone={result.analysis.status==="complete"?"success":"info"}>{result.analysis.status}</Badge><code className="text-xs">{result.revision}</code><Link href={`/repositories/${id}/explanations?ref=${result.revision}`} className="ml-auto text-sm font-semibold text-[var(--brand)] hover:underline">Ask about this evidence</Link></div><p className="mt-2 text-sm text-[var(--muted)]">{result.analysis.method} · {result.analysis.files_scanned} files · {result.analysis.bytes_scanned.toLocaleString()} bytes. {result.analysis.reason}</p></Card>
