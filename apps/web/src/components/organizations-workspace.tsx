...
function AgentEvaluationWorkspace({ group, repositories, token }: { group: Organization; repositories: Repository[]; token: string | null }) {
  ...
  return <section className="space-y-4"><header><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--brand)]">Agent evaluation lab</p><h2 className="mt-1 text-2xl font-bold">Representative trials before participation</h2><p className="mt-2 text-sm text-[var(--muted)]">Suites freeze sanitized scenarios at an exact commit. Sessions have no publish, secret, merge, environment, or network authority; hidden criteria never appear here.</p></header>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{runs.map(r=><Card key={r.id} className="p-5"><div className="flex flex-wrap gap-2"><Badge tone={r.correctness_passed&&r.policy_passed&&r.budget_passed&&!r.contaminated?"success":"warning"}>{r.review_status}</Badge><Badge>{r.trial_label.replaceAll("_"," ")}</Badge>{r.contaminated&&<Badge tone="warning">contamination reported</Badge>}</div><p className="mt-3 text-sm">Correctness {r.correctness_passed?"passed":"failed"} · policy {r.policy_passed?"passed":"failed"} · budget {r.budget_passed?"passed":"failed"}</p><p className="mt-2 text-xs">{r.cost} cost · {r.latency_ms} ms · input <code>{r.input_digest.slice(0,12)}</code></p><p className="mt-2 text-xs text-[var(--muted)]">Authority: publish false · secrets false · merge false · environments false · network false</p><div className="mt-3 flex gap-2"><Button onClick={()=>void decide(r.id,"approved")}>Approve evidence</Button><Button variant="secondary" onClick={()=>void decide(r.id,"needs_work")}>Needs work</Button></div></Card>)}</div>
  </section>;
}
