...
export function OrganizationDetail({
  ...
  async function submit(path: string, options: RequestInit) {
    try {
      ...
      // Reconcile compare-and-swap conflicts with the authoritative profile
      // version while retaining the submitted form values for a retry.
      await load();
      setError(message(e));
      return false;
    }
  ...
  return (
    <div className="space-y-7">
      ...
      <StewardshipWorkspace
        ...
        submit={submit}
      />
      <AgentMatchingWorkspace group={group} repositories={portfolio.repositories} token={token} />
      <AgentEvaluationWorkspace group={group} repositories={portfolio.repositories} token={token} />
      <AgentProfileWorkspace group={group} owner={owner} submit={submit} />
...
function AgentEvaluationWorkspace({ group, repositories, token }: { group: Organization; repositories: Repository[]; token: string | null }) {
  const [suites,setSuites]=useState<AgentEvaluationSuite[]>([]),[runs,setRuns]=useState<AgentEvaluationRun[]>([]),[error,setError]=useState("");
  const load=useCallback(async()=>{if(!token)return;try{const [a,b]=await Promise.all([api<{suites:AgentEvaluationSuite[]}>(`/organizations/${group.id}/agent-evaluation-suites`,{},token),api<{runs:AgentEvaluationRun[]}>(`/organizations/${group.id}/agent-evaluation-runs`,{},token)]);setSuites(a.suites??[]);setRuns(b.runs??[]);setError("")}catch(reason){setError(message(reason))}},[group.id,token]);
  useEffect(()=>{void Promise.resolve().then(load)},[load]);
  async function create(event:FormEvent<HTMLFormElement>){event.preventDefault();const f=event.currentTarget,d=new FormData(f),scenario=String(d.get("scenario_id"));try{await api(`/organizations/${group.id}/agent-evaluation-suites`,{method:"POST",body:JSON.stringify({name:d.get("name"),repository_id:d.get("repository_id"),expected_version:0,revision:{repository_revision:d.get("revision"),scenarios:[{id:scenario,title:d.get("title"),visibility:d.get("visibility"),source:{kind:d.get("source_kind"),id:d.get("source_id"),revision:d.get("source_revision"),sanitized:true},inputs:[d.get("input")],permitted_context:[d.get("context")],sanitized_prompt:d.get("prompt"),expected_outcomes:[d.get("outcome")],rubric:[d.get("rubric")],uncertainty:[d.get("uncertainty")],human_judgment:[d.get("judgment")],training_use:d.get("training_use"),data_classification:d.get("data_classification"),license:d.get("license"),checks:[{name:"expected outcome",kind:"contains",expected:d.get("expected")}],hidden_checks:[{name:"protected regression",kind:"contains",expected:d.get("hidden_expected")}]}],budget:{max_cost:Number(d.get("cost")),max_latency_ms:Number(d.get("latency")),max_tool_actions:Number(d.get("actions"))},prohibited_actions:["publish","read_secret","merge","environment_write"],human_review_criteria:[d.get("review")],change_summary:"Initial representative project trial"}})},token);f.reset();await load()}catch(reason){setError(message(reason))}}
  async function run(event:FormEvent<HTMLFormElement>,suite:AgentEvaluationSuite){event.preventDefault();const f=event.currentTarget,d=new FormData(f),scenario=suite.revisions.at(-1)!.scenarios[0];try{await api(`/organizations/${group.id}/agent-evaluation-suites/${suite.id}/runs`,{method:"POST",body:JSON.stringify({suite_version:suite.revisions.length,trial:{agent_id:d.get("agent_id"),agent_profile_version:Number(d.get("profile_version")),operator_supplied:d.get("operator_supplied")==="on",outputs:{[scenario.id]:d.get("output")},tool_actions:[],artifacts:[],cost:Number(d.get("cost")),latency_ms:Number(d.get("latency")),failure:""}})},token);f.reset();await load()}catch(reason){setError(message(reason))}}
  async function decide(runID:string,decision:string){try{await api(`/organizations/${group.id}/agent-evaluation-runs/${runID}/decisions`,{method:"POST",body:JSON.stringify({decision,rationale:"Maintainer reviewed correctness, safety, artifacts, and failure evidence."})},token);await load()}catch(reason){setError(message(reason))}}
  return <section className="space-y-4"><header><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--brand)]">Agent evaluation lab</p><h2 className="mt-1 text-2xl font-bold">Team-owned expectations before participation</h2><p className="mt-2 text-sm text-[var(--muted)]">Turn governed work into reusable, versioned evidence without copying its private source. Protected cases and answer keys stay evaluator-only; cases prohibit implicit training use and grant no publish, secret, merge, environment, or network authority.</p></header>
    <Card className="p-5"><details><summary className="cursor-pointer font-semibold">Define evaluation suite</summary><form onSubmit={create} className="mt-4 grid gap-3 md:grid-cols-2"><input name="name" required placeholder="Suite name" className="min-h-10 rounded-lg border px-3"/><select name="repository_id" required className="min-h-10 rounded-lg border px-3"><option value="">Repository</option>{repositories.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><input name="revision" required minLength={40} maxLength={64} placeholder="Exact commit" className="min-h-10 rounded-lg border px-3 font-mono"/><input name="scenario_id" required placeholder="Scenario key" className="min-h-10 rounded-lg border px-3"/><input name="title" required placeholder="Scenario title" className="min-h-10 rounded-lg border px-3"/><select name="visibility" required className="min-h-10 rounded-lg border px-3"><option value="public">Public case</option><option value="protected">Protected answer set</option></select><select name="source_kind" required className="min-h-10 rounded-lg border px-3">{[["issue","Issue"],["support_thread","Support thread"],["task","Task"],["incident","Incident"],["decision","Decision"],["prior_session","Sanitized prior session"]].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><input name="source_id" required placeholder="Governed source ID" className="min-h-10 rounded-lg border px-3"/><input name="source_revision" placeholder="Source version or revision" className="min-h-10 rounded-lg border px-3"/><input name="input" required placeholder="Synthetic or sanitized input" className="min-h-10 rounded-lg border px-3"/><input name="context" required placeholder="Permitted context only" className="min-h-10 rounded-lg border px-3"/><textarea name="prompt" required placeholder="Sanitized repository scenario" className="rounded-lg border p-3"/><input name="outcome" required placeholder="Expected outcome" className="min-h-10 rounded-lg border px-3"/><input name="rubric" required placeholder="Quality and safety rubric" className="min-h-10 rounded-lg border px-3"/><input name="uncertainty" required placeholder="Known uncertainty" className="min-h-10 rounded-lg border px-3"/><input name="judgment" required placeholder="Decision reserved for a human" className="min-h-10 rounded-lg border px-3"/><select name="training_use" required className="min-h-10 rounded-lg border px-3"><option value="prohibited">Training prohibited</option><option value="explicit_consent_required">Explicit consent required</option></select><select name="data_classification" required className="min-h-10 rounded-lg border px-3"><option value="synthetic">Synthetic data</option><option value="sanitized">Sanitized source data</option></select><input name="license" required placeholder="License / evaluation-use terms" className="min-h-10 rounded-lg border px-3"/><input name="expected" required placeholder="Public output criterion" className="min-h-10 rounded-lg border px-3"/><input name="hidden_expected" required placeholder="Protected evaluator criterion" className="min-h-10 rounded-lg border px-3"/><input name="review" required placeholder="Human-review criterion" className="min-h-10 rounded-lg border px-3"/><input name="cost" required type="number" min="0" step=".01" placeholder="Maximum cost" className="min-h-10 rounded-lg border px-3"/><input name="latency" required type="number" min="1" placeholder="Maximum latency ms" className="min-h-10 rounded-lg border px-3"/><input name="actions" required type="number" min="1" placeholder="Maximum tool actions" className="min-h-10 rounded-lg border px-3"/><Button>Create version 1</Button></form></details>{error&&<p role="alert" className="mt-3 text-sm text-[var(--danger)]">{error}</p>}</Card>
    <div className="grid gap-4 lg:grid-cols-2">{suites.map(s=>{const v=s.revisions.at(-1)!;return <Card key={s.id} className="p-5"><div className="flex gap-2"><h3 className="font-semibold">{s.name}</h3><Badge>v{v.version}</Badge></div><p className="mt-2 text-xs font-mono">commit {v.repository_revision}</p><p className="mt-3 text-sm">{v.scenarios.length} sanitized scenario(s) · {v.prohibited_actions.length} prohibited actions · budget {v.budget.max_cost} cost / {v.budget.max_latency_ms} ms</p><details className="mt-4"><summary className="cursor-pointer text-sm font-semibold">Run isolated candidate</summary><form onSubmit={e=>void run(e,s)} className="mt-3 grid gap-2"><select name="agent_id" required className="min-h-10 rounded-lg border px-3"><option value="">Candidate agent</option>{group.agents.filter(a=>a.profiles.length).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><input name="profile_version" required type="number" min="1" placeholder="Exact profile version" className="min-h-10 rounded-lg border px-3"/><textarea name="output" required placeholder="Captured candidate output" className="rounded-lg border p-3"/><input name="cost" type="number" min="0" step=".01" required placeholder="Observed cost" className="min-h-10 rounded-lg border px-3"/><input name="latency" type="number" min="0" required placeholder="Observed latency ms" className="min-h-10 rounded-lg border px-3"/><label className="text-sm"><input name="operator_supplied" type="checkbox" className="mr-2"/>Operator-supplied trial</label><Button>Record isolated trial</Button></form></details></Card>})}</div>
    <div className="grid gap-4 lg:grid-cols-2">{runs.map(r=>{const successful=r.correctness_passed&&r.policy_passed&&r.budget_passed&&!r.contaminated&&!r.failure;return <Card key={r.id} className="p-5"><div className="flex flex-wrap gap-2"><Badge tone={successful?"success":"warning"}>{r.review_status}</Badge><Badge>{r.trial_label.replaceAll("_"," ")}</Badge>{r.contaminated&&<Badge tone="warning">contamination reported</Badge>}{r.failure&&<Badge tone="warning">execution failed</Badge>}</div><p className="mt-3 text-sm">Correctness {r.correctness_passed?"passed":"failed"} · policy {r.policy_passed?"passed":"failed"} · budget {r.budget_passed?"passed":"failed"}</p>{r.failure&&<p role="alert" className="mt-2 text-sm text-[var(--danger)]">Failure: {r.failure}</p>}<p className="mt-2 text-xs">{r.cost} cost · {r.latency_ms} ms · input <code>{r.input_digest.slice(0,12)}</code></p><p className="mt-2 text-xs text-[var(--muted)]">Authority: publish false · secrets false · merge false · environments false · network false</p><div className="mt-3 flex gap-2">{successful&&<Button onClick={()=>void decide(r.id,"approved")}>Approve evidence</Button>}<Button variant="secondary" onClick={()=>void decide(r.id,"needs_work")}>Needs work</Button></div></Card>})}</div>
  </section>;
}
function AgentMatchingWorkspace({ group, repositories, token }: { group: Organization; repositories: Repository[]; token: string | null }) {
  const [matches, setMatches] = useState<AgentMatchSet | null>(null);
  ...
  async function search(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<AgentMatchSet>(`/organizations/${group.id}/agent-matches`, { method: "POST", body: JSON.stringify({ source_kind: data.get("source_kind"), source_id: data.get("source_id"), repository_id: repositoryID, workflow: data.get("workflow"), deployment_boundary: data.get("deployment_boundary"), required_permissions: repositoryID ? [{ kind: "repository", id: repositoryID }] : [] }) }, token);
      setMatches(result); setError("");
    } catch (reason) { setError(message(reason)); }
  }
  const visible = matches?.matches.filter((match) => !eligibleOnly || match.eligible) ?? [];
  return <section className="space-y-4"><header><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--brand)]">Agent collaborator discovery</p><h2 className="mt-1 text-2xl font-bold">Compare agents against the work</h2><p className="mt-2 text-sm text-[var(--muted)]">Matching is deterministic and evidence-led. Searching does not assign an agent or grant authority, and source content is never copied into discovery results.</p></header>
    <Card className="p-5"><form className="grid gap-3 md:grid-cols-2 xl:grid-cols-5" onSubmit={search}><label className="text-xs font-semibold">Work source<select name="source_kind" className="mt-1 min-h-10 w-full rounded-lg border border-[var(--line-strong)] px-3">{[["task","Task"],["proposal","Proposal"],["issue","Issue"],["decision","Decision"],["incident","Incident"],["stewardship_mandate","Stewardship mandate"],["team_role","Team role"]].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="text-xs font-semibold">Source ID<input name="source_id" required maxLength={200} className="mt-1 min-h-10 w-full rounded-lg border border-[var(--line-strong)] px-3" /></label><label className="text-xs font-semibold">Supported workflow<input name="workflow" required maxLength={300} placeholder="incident response" className="mt-1 min-h-10 w-full rounded-lg border border-[var(--line-strong)] px-3" /></label><label className="text-xs font-semibold">Repository permission<select name="repository_id" className="mt-1 min-h-10 w-full rounded-lg border border-[var(--line-strong)] px-3"><option value="">No repository required</option>{repositories.map((repo)=><option key={repo.id} value={repo.id}>{repo.name}</option>)}</select></label><label className="text-xs font-semibold">Deployment boundary<select name="deployment_boundary" className="mt-1 min-h-10 w-full rounded-lg border border-[var(--line-strong)] px-3"><option value="">Any disclosed boundary</option><option value="platform">Platform managed</option><option value="operator_managed">Operator managed</option><option value="customer_managed">Customer managed</option><option value="external_service">External service</option></select></label><Button type="submit">Explain matches</Button></form>{error&&<p role="alert" className="mt-3 text-sm text-[var(--danger)]">{error}</p>}</Card>
    {matches&&<><div className="flex flex-wrap items-center justify-between gap-3"><p className="max-w-4xl text-sm text-[var(--muted)]">{matches.explanation}</p><label className="text-sm"><input type="checkbox" checked={eligibleOnly} onChange={(event)=>setEligibleOnly(event.target.checked)} className="mr-2"/>Eligible only</label></div><div className="grid gap-4 lg:grid-cols-2">{visible.map((match)=><Card key={match.agent_id} className="p-5"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{match.name}</h3><Badge tone={match.eligible?"success":"warning"}>{match.eligible?"plausible":"not currently eligible"}</Badge><Badge>evidence score {match.score}</Badge></div><p className="mt-3 text-sm"><strong>Cost / availability:</strong> {match.pricing||"not disclosed"} · {match.availability||"not disclosed"}</p><p className="mt-2 text-sm"><strong>Effective permission:</strong> {match.effective_permissions.map((x)=>`${x.kind}:${x.id}`).join(" · ")||"none"}</p><p className="mt-2 text-sm"><strong>Deployment:</strong> {match.deployment_boundary.join(" · ")||"not disclosed"}</p>{match.reasons.map((reason)=><p key={reason} className="mt-2 text-sm text-[var(--success)]">✓ {reason}</p>)}{match.conflicts.map((item)=><p key={item} className="mt-2 text-sm text-[var(--danger)]">Conflict: {item}</p>)}{match.missing_evidence.map((item)=><p key={item} className="mt-2 text-sm text-[var(--muted)]">Missing: {item}</p>)}{match.stale_evidence.map((item)=><p key={item} className="mt-2 text-sm text-[var(--warning)]">Stale: {item}</p>)}<details className="mt-4"><summary className="cursor-pointer text-sm font-semibold">Verified evaluations and comparable outcomes</summary>{match.verified_evaluations.map((item)=><p key={`${item.kind}-${item.verified_at}`} className="mt-2 text-xs">{item.statement} · {new Date(item.verified_at).toLocaleDateString()}</p>)}{match.comparable_outcomes.map((item)=><p key={item.id} className="mt-2 text-xs">{item.status}: {item.summary}</p>)}{match.verified_evaluations.length===0&&match.comparable_outcomes.length===0&&<p className="mt-2 text-xs text-[var(--muted)]">No retained evidence is available.</p>}</details></Card>)}</div></>}
  </section>;
}
function AgentProfileWorkspace({ group, owner, submit }: { group: Organization; owner: boolean; submit: (path: string, init: RequestInit) => Promise<boolean> }) {
  const split = (value: FormDataEntryValue | null) => String(value ?? "").split("\n").map((x) => x.trim()).filter(Boolean);
  return <section className="space-y-4"><header><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--brand)]">Agent transparency</p><h2 className="mt-1 text-2xl font-bold">Published agent profiles</h2><p className="mt-2 text-sm text-[var(--muted)]">Operator claims are versioned separately from platform-verified identity evidence. A profile grants no access or authority.</p></header>
    <div className="grid gap-4 lg:grid-cols-2">{group.agents.map((agent) => { const current = agent.profiles?.at(-1); return <Card key={agent.id} className="p-5"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{agent.name}</h3><Badge>agent:{agent.id}</Badge>{current&&<Badge tone="info">profile v{current.version}</Badge>}</div>{current ? <div className="mt-4 space-y-3 text-sm"><p>{current.summary}</p><p><strong>Supported tasks:</strong> {current.supported_tasks.join(" · ")}</p><p><strong>Tools:</strong> {current.tools.join(" · ") || "None disclosed"}</p><p><strong>Model:</strong> {current.model_provenance}</p><p><strong>Execution:</strong> {current.execution_provenance}</p><p><strong>Structured deployment:</strong> {current.deployment_boundaries.join(" · ") || "None disclosed"}</p><p><strong>Project data:</strong> {current.data_use} <strong>Retention:</strong> {current.retention}</p><p><strong>Subprocessors:</strong> {current.subprocessors.join(" · ") || "None disclosed"}</p><p><strong>Remote boundaries:</strong> {current.remote_execution_boundaries.join(" · ") || "None disclosed"}</p><p><strong>Cost/resources:</strong> {current.pricing}; {current.resource_requirements.join(" · ") || "none"}</p><p><strong>Requested capabilities:</strong> {current.requested_capabilities.join(" · ") || "none"}</p><p><strong>Availability/support:</strong> {current.availability}; {current.support}</p><div className="rounded-lg border border-[var(--line)] p-3"><strong>Platform-verified evidence</strong>{current.verified_evidence.map((e)=><p key={e.kind} className="mt-1 text-xs">{e.statement}</p>)}</div><p className="text-xs text-[var(--muted)]">Changed by {current.published_by}: {current.change_summary} · {new Date(current.published_at).toLocaleString()}</p></div> : <p className="mt-3 text-sm text-[var(--muted)]">No profile has been published.</p>}
      {owner&&<details className="mt-5"><summary className="cursor-pointer font-semibold">Publish successor profile</summary><form className="mt-3 grid gap-3" onSubmit={(e)=>{e.preventDefault();const f=e.currentTarget,d=new FormData(f);void submit(`/organizations/${group.id}/agents/${agent.id}/profile`,{method:"PUT",body:JSON.stringify({expected_version:agent.profiles?.length??0,profile:{summary:d.get("summary"),supported_tasks:split(d.get("supported_tasks")),tools:split(d.get("tools")),model_provenance:d.get("model_provenance"),execution_provenance:d.get("execution_provenance"),deployment_boundaries:split(d.get("deployment_boundaries")),data_use:d.get("data_use"),retention:d.get("retention"),pricing:d.get("pricing"),resource_requirements:split(d.get("resource_requirements")),requested_capabilities:split(d.get("requested_capabilities")),availability:d.get("availability"),support:d.get("support"),subprocessors:split(d.get("subprocessors")),remote_execution_boundaries:split(d.get("remote_execution_boundaries")),conflict_disclosures:split(d.get("conflict_disclosures")),change_summary:d.get("change_summary")}})}).then((published)=>{if(published)f.reset()})}}>{[["summary","Summary"],["supported_tasks","Supported tasks (one per line)"],["tools","Tools (one per line)"],["model_provenance","Model provenance"],["execution_provenance","Execution provenance"],["deployment_boundaries","Structured deployment boundaries: platform, operator_managed, customer_managed, external_service"],["data_use","Project-context data use"],["retention","Retention and deletion"],["pricing","Pricing or requirements"],["resource_requirements","Resources (one per line)"],["requested_capabilities","Requested capabilities (one per line)"],["availability","Availability"],["support","Support and escalation"],["subprocessors","Subprocessors and purpose (one per line)"],["remote_execution_boundaries","Remote execution boundaries (one per line)"],["conflict_disclosures","Conflicts of interest (one per line)"],["change_summary","What changed"]].map(([name,label])=><label key={name} className="text-xs font-semibold">{label}<textarea name={name} required={!(["tools","resource_requirements","requested_capabilities","deployment_boundaries","subprocessors","remote_execution_boundaries","conflict_disclosures"].includes(name))} rows={2} className="mt-1 w-full rounded-lg border border-[var(--line-strong)] p-2 font-normal"/></label>)}<Button>Publish version {agent.profiles?.length+1||1}</Button></form></details>}</Card>})}</div></section>;
}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        {directory.agents.map((agent) => (
          <Card key={agent.id} className="p-5">
            ...
            {agent.profiles?.at(-1) && (() => { const profile = agent.profiles.at(-1)!; return <div className="mt-4 border-t border-[var(--line)] pt-3 text-sm"><p><strong>Profile v{profile.version}:</strong> {profile.summary}</p><p className="mt-2"><strong>Tasks/tools:</strong> {profile.supported_tasks.join(" · ")} / {profile.tools.join(" · ") || "none disclosed"}</p><p className="mt-2"><strong>Model/execution:</strong> {profile.model_provenance}; {profile.execution_provenance}</p><p className="mt-2"><strong>Data/retention:</strong> {profile.data_use}; {profile.retention}</p><p className="mt-2"><strong>Remote processing:</strong> {profile.subprocessors.join(" · ") || "no subprocessors disclosed"}; {profile.remote_execution_boundaries.join(" · ") || "no remote boundary disclosed"}</p><p className="mt-2"><strong>Cost, availability, support:</strong> {profile.pricing}; {profile.availability}; {profile.support}</p><p className="mt-2"><strong>Capabilities requested (not granted):</strong> {profile.requested_capabilities.join(" · ") || "none"}</p><div className="mt-3 rounded-lg border border-[var(--line)] p-2"><strong>Platform verified</strong>{profile.verified_evidence.map((e)=><p key={e.kind} className="mt-1 text-xs">{e.statement}</p>)}</div><p className="mt-2 text-xs text-[var(--muted)]">Change: {profile.change_summary}</p></div>; })()}
          </Card>
        ))}
      </div>
