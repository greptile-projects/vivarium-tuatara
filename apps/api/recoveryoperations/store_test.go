...
func TestRecoveryRequiresIndependentApprovalAndDependencyValidation(t *testing.T) {
	...
	if _, err = s.UpdateStep(v.ID, "data", "responder", "running", "starting", v.CurrentVersion); err != ErrConflict {
	...
	if _, err = s.UpdateStep(v.ID, "service", "agent", "running", "starting", v.CurrentVersion); err != ErrConflict {
		t.Fatalf("dependency bypass = %v", err)
	}
	v, err = s.UpdateStep(v.ID, "data", "responder", "running", "isolated restore", v.CurrentVersion)
	if err != nil {
		t.Fatal(err)
	}
	v, err = s.UpdateStep(v.ID, "data", "responder", "validated", "manifest matches", v.CurrentVersion)
	if err != nil {
		t.Fatal(err)
	}
	v, err = s.UpdateStep(v.ID, "service", "agent", "running", "delegated bootstrap", v.CurrentVersion)
	if err != nil {
		t.Fatal(err)
	}
	v, err = s.UpdateStep(v.ID, "service", "agent", "failed", "health journey failed", v.CurrentVersion)
