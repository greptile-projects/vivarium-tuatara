...
func (e *Executor) Execute(repositoryID, id string) error {
	...
	mountUser := fmt.Sprintf("%d:%d", os.Getuid(), os.Getgid())
	...
	command := exec.CommandContext(commandContext, "docker", "run", "--rm", "--user", mountUser, "--cap-drop=ALL", "--security-opt=no-new-privileges", "--read-only", "--tmpfs", "/tmp:rw,noexec,nosuid,size=16m", "--env-file", envName, "--mount", "type=bind,src="+mountName+",dst=/vivarium/artifact,readonly", environment.Image, "sh", "-c", environment.Command)
	...
	for stageIndex, stage := range promotion.Rollout.Stages {
		...
		for _, signal := range stage.Signals {
			...
			signalCommand := exec.CommandContext(signalContext, "docker", "run", "--rm", "--user", mountUser, "--cap-drop=ALL", "--security-opt=no-new-privileges", "--read-only", "--tmpfs", "/tmp:rw,noexec,nosuid,size=16m", "--env-file", envName, "--mount", "type=bind,src="+mountName+",dst=/vivarium/artifact,readonly", environment.Image, "sh", "-c", signal.Command)
