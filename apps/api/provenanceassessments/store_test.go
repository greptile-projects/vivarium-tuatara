...
func TestOwnerLinksCleanRoomRepairWithFrozenContext(t *testing.T) {
	...
	restricted, err := s.LinkRepair("repo", a.ID, "owner", out.Version, Repair{RequestID: "governed", WorkDigest: strings.Repeat("f", 64), FindingID: "origin", AffectedRevision: revision, Strategy: "obtain_permission", PermittedEvidence: []EvidenceReference{{Kind: "legal_record", ResourceID: "confidential-case-123", Revision: revision, Access: "restricted"}}, AcceptanceCriteria: []string{"permission retained"}, ProposalID: "proposal-4", TaskIDs: []string{"task-4"}}, func() Current { return current })
	...
	_, err = s.LinkRepair("repo", a.ID, "reviewer", restricted.Version, Repair{RequestID: "unauthorized", WorkDigest: strings.Repeat("a", 64), FindingID: "origin", AffectedRevision: revision, Strategy: "remove", AcceptanceCriteria: []string{"removed"}, ProposalID: "proposal-3", TaskIDs: []string{"task-3"}}, func() Current { return current })
	if err != ErrForbidden {
