...
func workflowRepositoryHead(git *storage.Store, repo, defaultBranch string) string {
	if git == nil || strings.TrimSpace(defaultBranch) == "" {
		return ""
	}
	r, err := git.Open(repo)
	if err != nil {
	...
	out, err := exec.Command("git", "--git-dir="+r.Path(), "rev-parse", "refs/heads/"+defaultBranch).Output()
	if err != nil {
		return ""
	}
