...
func TestVersionHistoryAndExplicitDiagnostics(t *testing.T) {
	...
	if len(first.Diagnostics) == 0 {
		t.Fatal("missing owners and stale implementation must remain explicit")
	}
...
func TestConflictingCurrentDefinitionsAreProjected(t *testing.T) {
	s, _ := New(t.TempDir())
	a, _ := s.Create("repo", "one", revision("Button"))
	created, err := s.Create("repo", "two", revision("Button"))
	if err != nil {
		t.Fatal(err)
	}
...
func hasDiagnostic(system System, kind string) bool {
	for _, diagnostic := range system.Diagnostics {
