...
func main() {
	...
	observabilityGapRoot := os.Getenv("OBSERVABILITY_GAP_STORAGE_ROOT")
	if observabilityGapRoot == "" {
		observabilityGapRoot = "observability-gaps"
	}
	observabilityGapStore, err := observabilitygaps.New(observabilityGapRoot)
	if err != nil {
		log.Fatal(err)
	}
	adoptionCampaignRoot := os.Getenv("ADOPTION_CAMPAIGN_STORAGE_ROOT")
	if adoptionCampaignRoot == "" {
		adoptionCampaignRoot = "adoption-campaigns"
	}
	adoptionCampaignStore, err := adoptioncampaigns.New(adoptionCampaignRoot)
	if err != nil {
		log.Fatal(err)
	}
	telemetryContractRoot := os.Getenv("TELEMETRY_CONTRACT_STORAGE_ROOT")
	if telemetryContractRoot == "" {
		telemetryContractRoot = "telemetry-contracts"
	}
	telemetryContractStore, err := telemetrycontracts.New(telemetryContractRoot)
	if err != nil {
		log.Fatal(err)
	}
	signalRolloutRoot := os.Getenv("SIGNAL_ROLLOUT_STORAGE_ROOT")
	...
	capacityTestRoot := os.Getenv("CAPACITY_TEST_STORAGE_ROOT")
	if capacityTestRoot == "" {
		capacityTestRoot = "capacity-tests"
	}
	capacityTestStore, err := capacitytests.New(capacityTestRoot)
	if err != nil {
		log.Fatal(err)
	}
	capacityPlanRoot := os.Getenv("CAPACITY_PLAN_STORAGE_ROOT")
	if capacityPlanRoot == "" {
		capacityPlanRoot = "capacity-plans"
	}
	...
	responsePolicyRoot := os.Getenv("RESPONSE_POLICY_STORAGE_ROOT")
	if responsePolicyRoot == "" {
		responsePolicyRoot = "response-policies"
	}
	responsePolicyStore, err := responsepolicies.New(responsePolicyRoot)
	if err != nil {
		log.Fatal(err)
	}
	runbookRoot := os.Getenv("RUNBOOK_STORAGE_ROOT")
	if runbookRoot == "" {
		runbookRoot = "runbooks"
	}
	runbookStore, err := runbooks.New(runbookRoot)
	if err != nil {
		log.Fatal(err)
	}
	responseAlertRoot := os.Getenv("RESPONSE_ALERT_STORAGE_ROOT")
	if responseAlertRoot == "" {
		responseAlertRoot = "response-alerts"
	}
	responseAlertStore, err := responsealerts.New(responseAlertRoot)
	if err != nil {
		log.Fatal(err)
	}
...
func newPlatformHandlerWithChecks(store *storage.Store, userStore *users.Store, authStore *auth.Store, repositoryCatalog *repositories.Store, proposalStore *proposals.Store, pullRequestStore *pullrequests.Store, activityStore *activities.Store, changeSessionStore *changesessions.Store, checkRunStore *checkruns.Store, optionalStores ...any) http.Handler {
	...
	var responsePolicyStore *responsepolicies.Store
	var responseAlertStore *responsealerts.Store
	var runbookStore *runbooks.Store
	...
	var adoptionCampaignStore *adoptioncampaigns.Store
	for _, optional := range optionalStores {
		...
		case *capacitytests.Store:
			capacityTestStore = value
		case *capacityplans.Store:
			capacityPlanStore = value
		case *responsepolicies.Store:
			responsePolicyStore = value
		case *responsealerts.Store:
			responseAlertStore = value
		case *runbooks.Store:
			runbookStore = value
		case *observabilitygaps.Store:
			observabilityGapStore = value
		case *telemetrycontracts.Store:
		...
		case *adoptioncampaigns.Store:
			adoptionCampaignStore = value
	...
	if authStore != nil && repositoryCatalog != nil && responsePolicyStore != nil {
		...
		if responseAlertStore != nil {
			registerResponseAlertRoutes(mux, repositoryCatalog, authStore, responsePolicyStore, responseAlertStore, incidentStore, activityStore, proposalStore)
		}
	}
	if authStore != nil && repositoryCatalog != nil && runbookStore != nil {
		registerRunbookRoutes(mux, store, repositoryCatalog, authStore, runbookStore, workflowComponentStore, organizationStore, proposalStore, responseAlertStore)
	}
	...
	if authStore != nil && repositoryCatalog != nil && adoptionCampaignStore != nil && releaseStore != nil && provenanceBundleStore != nil {
		registerAdoptionCampaignRoutes(mux, repositoryCatalog, authStore, adoptionCampaignStore, releaseStore, provenanceBundleStore, provenanceGraphStore, provenancePolicyStore, packageStore, pullRequestStore, decisionStore, documentationStore, apiContractStore, durableSchemaStore)
	}
...
func classifyInboxEvent(userID, ownerID string, event activities.Event, proposalStore *proposals.Store, pullRequestStore *pullrequests.Store, incidentStore *incidents.Store) (string, string, error) {
	...
	if event.Kind == "response_alert.routed" && event.TargetUserID != nil && *event.TargetUserID == userID {
		return "response", "Acknowledge urgent signal", nil
	}
