...
func TestAcceptedDutyHandoffTransfersActiveAlertEventAuthority(t *testing.T) {
	...
	rotation := responsepolicies.Rotation{
		Revisions:      []responsepolicies.RotationRevision{{PolicyID: "policy", TeamID: "service", Shifts: []responsepolicies.Shift{{ID: "active", StartsAt: now.Add(-time.Hour), EndsAt: now.Add(time.Hour), PrimaryUserID: "primary", BackupUserIDs: []string{"backup"}}}}},
	...
	if !responseAlertEventAllowed(alert, "backup", []responsepolicies.Rotation{rotation}, participants) {
		t.Fatal("accepted effective owner could not complete the active alert")
	}
	unrelated := alert
