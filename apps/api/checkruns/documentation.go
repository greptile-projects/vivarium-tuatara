...
func ParseDocumentationConfig(data []byte, readPath func(string) ([]byte, error)) (DocumentationConfig, []Definition, error) {
	var config DocumentationConfig
	decoder := json.NewDecoder(strings.NewReader(string(data)))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&config); err != nil || config.Version != 1 || len(config.Checks) == 0 || len(config.Checks) > 20 {
	...
	for _, check := range config.Checks {
		...
		for _, target := range check.Targets {
			...
			if target.Version == "" || len(target.Version) > 40 || len(target.Revision) != 40 || !map[string]bool{"source": true, "package": true, "release": true}[target.Source] || seen[name] {
