...
func TestScenarioProvenanceResolvesExactBranchAssetsAndQualityIntent(t *testing.T) {
	...
	testBody, fixtureBody, rationale := []byte("test checkout"), []byte(`{"shopper":"synthetic"}`), []byte("expected behavior")
	...
	tree := writeTestTree(t, repo,
		...
		testTreeEntry{mode: "40000", name: "fixtures", id: writeTestTree(t, repo, testTreeEntry{mode: "100644", name: "shopper.json", id: fixtureBlob})},
	...
	plan, err := plans.Create("repo", "owner", qualityAPIRevision("owner"))
	if err != nil {
		t.Fatal(err)
	}
	sum := sha256.Sum256(fixtureBody)
	v := testscenarios.Scenario{Title: "Decline", Purpose: "Prove safe recovery", QualityPlanID: plan.ID, QualityPlanVersion: 1, RequirementIDs: []string{"pay"}, Sources: []testscenarios.Source{{Kind: "documentation", ResourceID: "journey", Revision: string(commit), Path: "docs/journey.md", Summary: "Reviewed journey"}}, Parameters: []testscenarios.Parameter{{Name: "decline", Description: "Decline response", Type: "boolean"}}, Preconditions: []testscenarios.Step{{ID: "ready", Description: "Service ready", Operation: "health check"}}, Actions: []testscenarios.Step{{ID: "submit", Description: "Submit", Operation: "request", Parameters: []string{"decline"}}}, Assertions: []testscenarios.Assertion{{ID: "safe", Description: "No order", Matcher: "equals", Expected: "0"}}, Fixtures: []testscenarios.Fixture{{ID: "shopper", Kind: "synthetic", Description: "Synthetic shopper", Path: "fixtures/shopper.json", SHA256: hex.EncodeToString(sum[:]), DataClass: "synthetic", Assumptions: []string{"No identity"}, SourceIDs: []string{"journey"}}}, Environments: []testscenarios.Environment{{ID: "local", Description: "Local", Runtime: "bun", Requirements: []string{"loopback"}}}, Cases: []testscenarios.Case{{ID: "decline", Name: "Decline", Values: map[string]string{"decline": "true"}, Assumptions: []string{"deterministic"}, ExpectedOutcome: "No order"}}, Implementation: testscenarios.Implementation{AuthoredByType: "human", Branch: "scenario/decline", CommitID: string(commit), TestPaths: []string{"tests/checkout.test.ts"}, Command: "bun test", Framework: "bun:test", Assumptions: []string{"local"}, Provenance: []string{"journey"}}}
	...
	if !testScenarioProvenance(git, "repo", v, plans, nil, nil) {
