...
func TestBoundedRehearsalRetainsExactSanitizedEvidence(t *testing.T) {
	...
	v, _ := s.Create("repo","owner",r); r.Definition="v2"; r.Rationale="candidate"; v,_=s.Revise("repo",v.ID,1,"owner",r)
	...
	v,_=s.AddMigration("repo",v.ID,"owner",m); migration:=v.Migrations[0]
	rehearsal:=Rehearsal{Name:"realistic tokenized rehearsal",ApplicationRevision:"commit",Dataset:RehearsalDataset{Kind:"representative",Description:"tokenized distribution",PrivacyMethod:"irreversible tokenization",Digest:"data-digest",MaxBytes:1024,RowCount:10},Dependencies:[]RehearsalDependency{{Name:"driver",Revision:"1.2.3",Digest:"dep-digest"}},Checks:[]RehearsalCheck{{ID:"upgrade",Kind:"upgrade",Command:"./check upgrade",Invariant:"balances unchanged",RevisionInputs:[]string{"application","schema_from","schema_to","migration","data_shape","dependency:driver"}},{ID:"rollback",Kind:"rollback",Command:"./check rollback",Invariant:"old reader succeeds",RevisionInputs:[]string{"application","migration"}}}}
	v,rehearsal,err:=s.CreateRehearsal("repo",v.ID,migration.ID,"owner",migration.Version,rehearsal);if err!=nil||len(v.Migrations[0].Rehearsals)!=1{t.Fatalf("create rehearsal = %#v, %v",rehearsal,err)}
	...
	v,run,err=s.AddRehearsalRun("repo",v.ID,migration.ID,rehearsal.ID,"agent",run);if err!=nil||run.ID==""{t.Fatalf("run = %#v, %v",run,err)}
	_,note,err:=s.AddRehearsalNote("repo",v.ID,migration.ID,rehearsal.ID,"owner",run.ID,"Rollback failed only for enum values; investigate decoder.");if err!=nil||note.ActorID!="owner"{t.Fatalf("note = %#v, %v",note,err)}
	bad:=run;bad.ID="";bad.Result="passed";if _,_,err=s.AddRehearsalRun("repo",v.ID,migration.ID,rehearsal.ID,"agent",bad);err!=ErrInvalid{t.Fatalf("dishonest passing summary accepted: %v",err)}
}
