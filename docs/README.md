...
## Revision-bound response alerts

Repository writers submit reliability, deployment, security, privacy, dependency, workflow,
and user-impact signals through `POST /repositories/{id}/response-alerts`. Each signal freezes
its source revision, occurrence time, affected resources and user groups, uncertainty, and
digest-addressed evidence. The server evaluates it against exactly one rule in the newest
response-policy revision, derives acknowledgement and resolution deadlines, and uses an active
...
links to roadmap, experiment, performance, reliability, infrastructure, release, and
funding work. Storage defaults to `$CAPACITY_OBJECTIVE_STORAGE_ROOT` or
`capacity-objectives`; JSON files are atomically replaced under the store's cross-process
lock while earlier revisions remain embedded and readable.
...
runs retain measured duration, request count, peak concurrency, throughput, percentile
latency, errors, saturation, recovery, correctness, resources, optional declared carbon,
cost, repetitions, variability, comparability, derived limit breaches, and a log digest.
Missing measurements and negative or non-finite metrics cannot prove capacity. Only
successful, correct, comparable evidence with at least
...
derives every limit breach from the matched scenario rather than trusting the submitter.
...
Current repository owners assign responsibility for those exact areas through the pull's `/review-assignments` routes. Suggestions consider current code ownership and participation, attributable project review, organization team responsibility, active assignment load, self-review and disclosed conflicts, availability, and approved agent profiles backed by a live exact-repository grant. Each explanation is limited to evidence the reader may already inspect. Invitations are caller-stable and permit only one invited or accepted reviewer per area; deadlines and escalation paths stay visible. Invitees can accept, decline, report unavailability, or recuse, while maintainers can release or replace them. Every non-active outcome carries explicit reassignment work, and reads project removed humans or revoked agent grants as revoked rather than treating historical acceptance as current. Acceptance is a bounded coordination assignment only and grants no repository, merge, secret, governance, policy, or operational authority.
...
## Revision-exact software provenance

Repository participants assemble immutable origin graphs through `POST /repositories/{id}/provenance-graphs` and reviewers inspect them through the list/detail API or repository `/provenance/graph` workspace. Each caller-stable request binds one exact commit and optional exact provenance-policy revision to typed files, commits, copied/generated fragments, assets, dependencies, build steps, artifacts, contributors, agents, upstream projects, licenses, attestations, and tools. Typed edges retain the claimed transformation, confidence, actor, tool, and citation rather than treating commit authorship or a scanner label as proof.

For repository-file citations, the server resolves the path from the exact commit tree and replaces caller identity and digest claims with the actual Git blob ID, commit, and SHA-256. The immutable analysis digest covers those normalized nodes and edges. Read projection compares the retained commit with the live default branch, distinguishes ordinary movement from history that is no longer reachable, and keeps missing material origins, unknown evidence, contradictory transformations, and incompatible license claims visible and attributable.

Nodes can be public, repository-visible, or restricted to named audience members. Outside a restricted audience, reads preserve the existence, kind, confidence, and graph gap while removing the source label, revision, license, obligations, citations, and incident edge citation/tool details. The ledger therefore supports private and embargoed inputs without copying their identities or terms to broader readers. Storage defaults beneath `$PROVENANCE_GRAPH_STORAGE_ROOT` (`provenance-graphs`). Records provide evidence only and grant no Git, package, build, release, review, agent, disclosure, or distribution authority.

## Candidate provenance readiness

Repository participants assess exact pull requests, stack integration candidates, package candidates, and release candidates through `POST /repositories/{id}/provenance-assessments` and the repository `/provenance/assessments` workspace. Each immutable assessment binds the candidate commit to one revision-exact provenance graph, one effective repository policy revision, and explicit distribution targets. The server derives unattributed or policy-uncovered material, incompatible or unknown licenses, generated-output review, required notices and source offers, contributor attestations, and accountable review owners rather than accepting caller-authored readiness findings.

Humans and repository-bound read-only agents append revision-bound challenges and cited origin evidence. Only current human policy or material-review owners can acknowledge a finding or retain a follow-up-backed exception expiring within 30 days. Projection independently rechecks the candidate, graph digest, dependency and tool revisions, and each affected material rule; an unrelated policy-rule change therefore preserves decisions it cannot affect. Once a repository has provenance policy, ordinary pull merge and integration-queue readiness requires an exact current assessment with every blocking finding resolved. Storage defaults beneath `$PROVENANCE_ASSESSMENT_STORAGE_ROOT` (`provenance-assessments`). Assessment evidence grants no Git, review, merge, package, release, disclosure, exception, or distribution authority.

The connected `software-provenance-journey.spec.ts` exercises the complete distribution loop through the web application, public API, stock Git, a containerized release build, and a standard HTTP package client. It retains local, federated, agent, generated, private, and transitive origins; blocks incompatible and unattributed material; routes both findings into ordinary governed replacement tasks; and proves current pull and release gates before package publication. A public consumer downloads the artifact and independently verifies the Ed25519 signature over the canonical claim digest. False matches, disputed authorship, a rejected non-owner exception, generator movement, revoked upstream attestation, and post-release origin discovery remain visible without widening private evidence or mutating the signed claim.
...
Every member pull exposes `GET /repositories/{id}/pulls/{pull_id}/stack-context`. That projection separates the exact commits and diff from the declared parent from the cumulative target diff, lists the exact upstream member/pull revisions, and classifies the layer as `reviewable_now`, `provisional`, or `blocked`. It joins ordinary comments, review decisions, checks, previews, and preview findings by their exact revisions; comments created after this capability freeze the pull source revision, while older comments without that fact remain visibly `unbound`. A current human repository owner may append an exact layer and complete-upstream-map acknowledgement through the context acknowledgement route. The pull page presents inherited cumulative code as context rather than another approval request and explicitly names all downstream layer evidence that an upstream replacement would stale.

After feedback, a current human contributor can push a revised member with stock Git and submit the complete desired membership/order to `POST /repositories/{id}/change-stacks/{stack_id}/restacks`. The retry-stable preview normalizes source/branch identities, rejects missing or cyclic dependencies, replays each member's retained individual commits onto its proposed predecessor in a disposable Git view, and reports exact old-to-new commit mappings, insertion/removal/reorder actions, conflicts, branch updates, and review/check invalidation before any authoritative mutation. `POST /repositories/{id}/change-stacks/{stack_id}/restacks/{restack_id}/apply` rechecks the frozen target, shared branch use, conflicts, every exact old branch tip, and deterministic candidates. It holds current repository participation against concurrent collaborator removal while importing rewritten objects, updating all same-repository member refs with one compare-and-swap Git transaction, and retaining lineage, so a concurrent stock push or access revocation updates none. Persistence independently refuses invalid dependency graphs. Missing or shared branches, rewrite failures, target movement, and independently owned cross-repository work remain actionable blockers. Successful application synchronizes ordinary pull source revisions and retains attributable old-to-new revision lineage; neither preview nor stack membership grants branch authority.

Parallel work stays layer-owned through the member assignment, work-launch, and timeline routes. A current human source participant assigns a layer to another current human participant or to an approved agent named by a live repository access grant. The active assignee may open a caller-stable `change_session`, `shared_workspace`, or `conflict_resolution_workspace`; the retained context freezes the stack outcome, member revision, parent revision, upstream revision map, acceptance criteria, and source/pull evidence. The assignee can publish revision-bound checkpoints, questions, handoffs, and—when human—proposed restacks to the common timeline. Stack reads compare every frozen upstream map with current member revisions and name changed assumptions without rewriting history. Cross-repository items and their coordination disappear when the caller lacks source read access, and repository-bound agents are rechecked against their live grant and exact member branch. These records coordinate work only and do not grant source, branch, fork, federation, embargo, workspace-runtime, pull, review, merge, or disclosure authority.

A current human target maintainer can reserve exact cumulative verification for a ready prefix through `POST /repositories/{id}/change-stacks/{stack_id}/integration-candidates`. The server first requires every included ordinary pull to remain open at its retained source revision and to satisfy its ordinary review, check, preview, and policy readiness. It then merges each layer in order into an unreferenced candidate rooted at the current target, freezes the target and complete member-revision map, resolves target-branch required-check definitions from that candidate, and starts those checks against its exact commit. Conflicts, stale or withdrawn layers, policy failures, and independently owned repositories fail closed. Caller-stable requests reuse the retained candidate; a later target-based candidate preserves but explicitly supersedes unlanded evidence rooted at the old target. Candidate preparation grants no queue or merge authority. Once cumulative checks pass, `POST /repositories/{id}/change-stacks/{stack_id}/integration-candidates/{candidate_id}/merge` rechecks current human maintainer authority, every exact open member pull, and ordinary readiness before compare-and-swapping the target from its frozen revision to the candidate. Successful publication closes only the included prefix pulls against the exact integrated commit and retains the merged candidate; a concurrent target advance changes nothing, and an interrupted response can reconcile the already-published candidate on retry. If a member branch synchronizes in the post-transaction metadata window, retry proves the retained reviewed revision belongs to the published candidate before freezing the pull back to that exact delivered identity. The `/stacks` workspace exposes preparation, evidence state, supersession, partial-prefix history, and the final landing action.

## Accountable review finding decisions

The pull request review-work response includes every retained finding with its append-only decision history and a server-derived current state. Repository contributors use `POST /repositories/{id}/pulls/{pull_id}/review-findings/resolutions` to classify, discuss, challenge, defer, or reaffirm a finding; the pull author and current repository owner can additionally accept, supersede, resolve, accept risk, or grant an exception that expires within 30 days. The original cited finding is never edited, and caller-stable request identities make ambiguous retries converge while changed reuse conflicts.

Decisions can cite an existing commit, proposal task or follow-up, pull change session, or repository workspace. Those identities resolve through their ordinary stores and do not launch or authorize work. Verification evidence uses the review citation boundary: repository-defined checks and targeted reproductions must be check runs for the same pull and exact current source revision, and only a succeeded cited run verifies a claimed resolution. After source movement, historical findings and decisions remain visible as stale until a current-revision event explicitly says the finding remains applicable or records its disposition. Duplicate references, superseded decisions, rationale, dissent, accepted risk, and exception expiry remain attributable. The finding ledger grants no Git, branch, task, agent, session, workspace, check, review, merge, exception, or operational authority.
