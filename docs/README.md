...
After feedback, a current human contributor can push a revised member with stock Git and submit the complete desired membership/order to `POST /repositories/{id}/change-stacks/{stack_id}/restacks`. The retry-stable preview replays each member's retained individual commits onto its proposed predecessor in a disposable Git view and reports exact old-to-new commit mappings, insertion/removal/reorder actions, conflicts, branch updates, and review/check invalidation before any authoritative mutation. `POST /repositories/{id}/change-stacks/{stack_id}/restacks/{restack_id}/apply` rechecks the frozen target, current permissions, shared branch use, conflicts, every exact old branch tip, and deterministic candidates. It then imports the rewritten objects and updates all same-repository member refs with one compare-and-swap Git transaction, so a concurrent stock push updates none. Revoked access, missing or shared branches, rewrite failures, target movement, and independently owned cross-repository work remain actionable blockers. Successful application synchronizes ordinary pull source revisions and retains attributable old-to-new revision lineage; neither preview nor stack membership grants branch authority.
