...
Current human policy owners can convert a current finding into retry-stable ordinary repair proposals using replace, reimplement, remove, obtain-permission, or isolate strategies. Publication first retains an authorized reservation while holding the current policy revision stable, then creates ordinary work and finalizes its link; retries recover reserved work without exposing an unlinked proposal. The retained repair freezes the affected revision, exact policy and rule, obligations, acceptance criteria, permitted evidence references, clean-room boundary, proposal, ordered human/agent tasks, and authorizer. Clean-room plans reject restricted evidence; proposal/task handoffs preserve original authorship and continue through ordinary branch, fork, session, workspace, review, check, merge, release, and distribution authority.
