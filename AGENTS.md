...
Live runbook recommendations and executions at repository `/runbook-recommendations` and `/runbook-executions` routes freeze an alert, incident, deployment, failed workflow, service objective, support thread, or manual observation together with its exact revision, affected resources, signal window, releases, environment snapshot, digest-addressed permitted evidence, timeline references, and audience. Explained scope matches never launch automatically; equal matches require an explicit responder choice. A caller-stable launch binds one immutable runbook revision, but caller-reported preconditions and access never establish readiness: absent authoritative verification remains blocked. Missing current rehearsal, stale procedure, blocking runbook diagnostics, unmet or unverified preconditions, unavailable or unverified access, duplicate ready origin, ambiguous matches, and secret-bearing or incomplete context fail visibly. Blocked attempts remain immutable audit history while a corrected successor may be evaluated. Executions coordinate guided response only and grant none of the referenced repository, evidence, environment, workflow, deployment, disclosure, or operational authority.
Exact runbook revisions can retain caller-stable rehearsals through their `/rehearsals` route. Current repository humans and exact repository-bound organization agents exercise complete procedures against isolated or exactly policy-approved environments using digest-addressed synthetic or permitted inputs. Each scenario retains branch rationale, reviewed command identities, bounded sanitized output, step timing, artifacts, cost, permissions, outcome, manual gaps, and explicit simulation or exclusion of changing steps. Reads preserve failed proof and project later runbook, scoped resource, policy, or expiring credential movement as attributable stale evidence. Rehearsals prove procedure only and grant no environment, credential, command, agent, repository, deployment, disclosure, spending, or operational authority.
Live guided runbook collaboration uses execution detail and `/runbook-executions/{execution_id}/actions` routes. Each execution projects its exact step, controller, participants, evidence, pending decision, scoped credentials, health, cost, blockers, rollback, and next action from an append-only receipt ledger. Every action is caller-stable and optimistic-version-bound; only the controller can steer effects, a distinct human must approve approval-gated work, optional skips are limited to non-changing communication steps, and agents can only analyze or perform an exact explicitly delegated step while their live repository grant is held through persistence. Blocked launches cannot perform work, stale/disconnected retries conflict without repeating effects, and pause, handoff, abort, and rollback never grant authority in referenced systems.
Terminal runbook executions use their `/assessment` and `/improvements` routes to freeze service outcome and procedure-fitness evidence without changing the revision that ran. Current declared runbook owners evaluate every declared health, containment, recovery, communication, and rollback criterion against retained digests and record deviations, manual work, failed steps, timing, access gaps, agent corrections, cost, and attributable participant feedback. Supported documentation, workflow, policy, infrastructure, or code findings can create caller-stable ordinary proposals and human- or agent-owned tasks; a durable finding reservation precedes work creation, retries reconcile the linked winner, and current repository participation is held across assessment or the complete reserve/create/link mutation. Delivery and successor runbook publication remain under their normal review boundaries, and a successor needs fresh exact-version rehearsal proof. Unsafe drift may suspend current use only with an active exact fallback revision whose current version, status, and passing rehearsal are rechecked under the same runbook lock that persists suspension; recommendations retain the suspension blocker and fallback identity. Assessments, suspension, and linked work grant no repository, proposal, task, rehearsal, deployment, environment, agent, spending, or operational authority.
The connected `operational-runbook-journey.spec.ts` browser/API/stock-Git journey proves the complete knowledge-to-recovered-service loop from reviewed human/agent rehearsal through an authoritatively verified revision-exact alert, bounded approval and mitigation, handoff, recovery assessment, ordinary reviewed code/runbook improvement, successor publication, and fresh rehearsal. Alert verification establishes only an open, effectively unpaused same-repository source-revision/resource/window binding and repository-read access; the newest store-wide sequenced outcome routing action controls pause/resume even when the alert state remains open. Acknowledged, suppressed, maintenance, routing-paused, resolved, stale-context, caller-asserted, duplicate, unsafe-agent, revoked-agent, and obsolete-rehearsal paths remain contained.
...
Accountable finding decisions at pull request `/review-findings/resolutions` routes and the same pull surface append caller-stable classification, discussion, acceptance, challenge, supersession, deferral, resolution, applicable, accepted-risk, and maximum-30-day exception events without rewriting the original cited finding. Human contributors may discuss and challenge; only the pull author or current repository owner can accept, supersede, resolve, accept risk, or grant an exception, and read-only agents cannot publish those decisions. Links resolve through ordinary repository commits, proposal tasks/follow-ups, pull change sessions, and repository workspaces. Current-candidate check or targeted-reproduction citations derive verification; source movement projects findings as stale until an attributable current-revision event says they remain applicable or resolves them. Duplicate, dissent, rationale, expiry, and superseded decision identities remain visible, while the ledger grants no branch, task, session, workspace, check, agent, approval, merge, exception, or operational authority.
Check and preview citations must belong to the same pull and exact source revision, while decision citations must explicitly source or affect that pull in the same repository. Work persistence rechecks the exact accepted assignment under the same repository-wide cross-process mutation lock used by assignment transitions, so release or replacement cannot race publication.
Current pull review readiness at `/review-readiness`, ordinary `/merge-readiness`, and the pull web surface joins the newest immutable plan with exact-area assignments, cited evidence, findings, accountable human decisions, owner acknowledgements, gaps, and stale ordinary approvals. Once a review plan exists, every planned area is merge-blocking until an accepted reviewer, all required current evidence, required owner acknowledgements, and current finding dispositions are present; pulls without a plan retain ordinary approval behavior. Specialized evidence citations must name the exact plan-area domain and cover every declared area path; generic or partial citations remain visible but satisfy no specialized requirement. Non-terminal finding discussion remains visible without erasing the newest applicable terminal disposition, while a later applicable, accepted, or deferred state reopens that disposition. Source or target movement makes only evidence bound to the moved exact plan stale, and finding exceptions remain maximum-30-day owner decisions that must link ordinary follow-up work. The matrix grants no review, approval, merge, policy, exception, repository, or operational authority.
The connected `review-orchestration-journey.spec.ts` browser/API/stock-Git journey proves the complete boundary from a cross-cutting code, security, accessibility, and consumer change through qualified parallel human and read-only-agent review, recusal and unavailable-area handoff, disputed conflicting findings, requested revision, stale approval and changed risk, current verification, integration queue, and merge. Read-only organization-agent credentials may publish only accepted exact-area review observations and discussions; they gain no repository-write authority. Owners can replace declined, recused, or unavailable reviewers and remain the acknowledgement backstop across every planned area without the ordinary reviewer-load threshold deadlocking a wide plan. Revoked grants immediately invalidate agent credentials and project reassignment work, while public review-work collections render empty dependencies, links, and evidence as arrays.

Software provenance policy at repository and organization `/provenance-policies` routes and the corresponding `/provenance` workspaces retains immutable acceptable-origin and licensing revisions beneath `$PROVENANCE_POLICY_STORAGE_ROOT` (`provenance-policies`). Rules cover source, generated code, assets, models, datasets, packages, and build inputs with attribution, contributor attestations, review owners, distribution contexts, linked contributor/agent/package/release/boundary resources, and expiring exceptions. Reads keep unknown licenses, conflicting permitted/prohibited terms, missing owners, and expiring exceptions explicit and attributable; records grant no Git, contribution, package, release, agent, federation, or distribution authority.
Signed release provenance at repository release `/provenance-bundles` routes, public `/provenance/bundles/{id}` reads and comparisons, and package-version `/provenance` reads retains immutable audience-filtered claims beneath `$PROVENANCE_BUNDLE_STORAGE_ROOT` (`provenance-bundles`). Only a repository owner can bind an exact release artifact set to its current graph, blocking-free assessment, policy, source/build attestations, dependency lineage, licenses, notices, declared omissions, and verification instructions. The canonical claim digest is signed with a durable Ed25519 key; public consumers need no repository access to verify a public claim or compare licenses and omissions. Current graph/policy/package trust is projected separately, and owner-published license, attestation, quarantine, drift, or origin-gap notices append without changing the signed payload; cited repairs and propagation campaigns must resolve in the same repository. Bundles grant no source, package, release, distribution, remediation, propagation, or repository authority.
The connected `software-provenance-journey.spec.ts` browser/API/stock-Git/package-client journey proves this boundary from mixed local, federated, agent-generated, private, and transitive inputs through incompatible-license and unattributed-fragment blocks, challenges, rejected contributor exception, governed human/agent replacement work, current review and release gates, public package retrieval, and independent Ed25519 verification. Generator movement, upstream-attestation revocation, and a post-release origin discovery remain appended recovery evidence without changing the signed release claim or exposing the private dependency.
Revision-exact software provenance at repository `/provenance-graphs` routes and the `/provenance/graph` workspace retains immutable caller-stable graphs beneath `$PROVENANCE_GRAPH_STORAGE_ROOT` (`provenance-graphs`). Graphs bind an exact commit and server-resolved file/blob SHA-256 citations to typed source, fragment, asset, dependency, build, artifact, contributor, agent, upstream, license, attestation, and tool nodes plus attributable transformations, confidence, obligations, and an optional exact policy revision. Reads recheck default-branch reachability and movement, report missing origins and contradictory claims, and redact restricted node identities, terms, citations, and incident edges outside their explicit audience. Graphs grant no Git, package, build, release, review, agent, disclosure, or distribution authority.
Candidate provenance assessment at repository `/provenance-assessments` routes and the `/provenance/assessments` workspace binds an exact pull, stack integration candidate, package, or release candidate to an exact graph and effective policy revision beneath `$PROVENANCE_ASSESSMENT_STORAGE_ROOT` (`provenance-assessments`). Server-derived findings cover missing origin and policy, incompatible licenses, generated output, notices, source offers, attestations, review owners, and distribution targets. Human and read-only-agent challenges plus cited origin evidence remain revision-bound; only current human policy owners acknowledge or grant maximum-30-day exceptions with follow-up. Candidate, graph, dependency, tool, and per-material policy-rule movement selectively stale affected decisions, and repositories with provenance policy require current blocking-free assessment in ordinary pull merge/queue readiness. Records grant no Git, review, merge, package, release, disclosure, exception, or distribution authority.
Current human policy owners can convert a current finding into retry-stable ordinary repair proposals using replace, reimplement, remove, obtain-permission, or isolate strategies. Publication first retains an authorized reservation with deterministic proposal/task identities while holding both current policy and repository participation stable, then creates that exact ordinary work and finalizes its status; retries recover reserved work and every visible proposal is already linked even if finalization fails. The retained repair freezes the affected revision, exact policy and rule, obligations, acceptance criteria, permitted evidence references, clean-room boundary, proposal, ordered human/agent tasks, and authorizer. Clean-room plans reject restricted evidence; proposal/task handoffs preserve original authorship and continue through ordinary branch, fork, session, workspace, review, check, merge, release, and distribution authority.

Change stacks at `/repositories/{id}/change-stacks` and the repository `/stacks` workspace retain a shared outcome, target branch, explicit member order and dependencies, per-change acceptance criteria, ordinary pull identity, and each exact published source revision beneath `$CHANGE_STACK_STORAGE_ROOT` (`change-stacks`). Caller-stable publication reserves and validates the stack before branch-only members publish as marker-bound ordinary pull requests whose target is the preceding layer; retries reuse both the reservation and any already-created pull, while pull-list or creation failures return a retryable non-success response and existing pulls retain their ordinary review boundary. Reads derive individual and cumulative Git scope, commit authors, base relationships, current caller permissions, and explicit cycle, missing dependency/commit, unrelated-history, duplicate-change, moved-revision, pull-mismatch, and inaccessible-branch diagnostics. Cross-repository ranges recursively copy bounded exact objects from the complete caller-authorized target and stack-prefix repository set into disposable Git views, including ancestry distributed across three or more stores, and never import into any authoritative store. Caller-stable `/restacks` previews accept a complete revised order after contributors push revisions with stock Git, normalize source/branch identities, validate the complete dependency graph, replay each attributable individual commit onto its proposed parent only in a disposable view, and expose exact rewrite maps, conflicts, removed/inserted/reordered layers, review/check invalidation, and branch updates. Apply rechecks target, open shared-branch use, conflicts, exact tips, and deterministic candidates, then holds current repository participation through object import, the single compare-and-swap Git ref transaction, and lineage persistence; concurrent pushes or collaborator revocation update none. Persistence independently refuses invalid dependency graphs. Cross-repository members remain an actionable independently-owned blocker. Applied stacks retain old-to-new revision lineage and synchronize ordinary pull source revisions; preview grants no Git or pull authority.
Target maintainers reserve caller-stable cumulative `/integration-candidates` only for prefixes whose ordinary pulls remain open at their retained revisions and pass ordinary review, checks, previews, and policy. Each immutable unreferenced candidate is assembled in order from the live target, freezes the complete member revision map, and runs target-required checks on its exact commit. Conflict, stale/withdrawn work, policy failure, and independent repository ownership fail closed; target movement preserves old evidence as explicitly superseded, and candidate preparation grants no queue or merge authority. A current human target maintainer may land a passing candidate through its `/merge` route; the route rechecks every exact member and ordinary readiness boundary, compare-and-swaps the target from the frozen base to the candidate, closes the included ordinary pulls at that exact integrated history, and retains the merged prefix. Concurrent target movement updates nothing, while retries reconcile a target that already reached the candidate. If a member branch synchronizes after the target transaction, reconciliation proves the retained reviewed revision is reachable from the published candidate and restores the ordinary pull to that exact delivered identity; unrelated stale revisions are refused.
...
After a complete passing rehearsal, a named human response owner can publish only the retained candidate with every required role threshold satisfied. A durable `publishing` intent activates the enforced stock-Git push pause before Git's ref transaction compare-and-swaps all selected old tips to attested replacements atomically; exact retries reconcile either phase, while any independently moved ref aborts the whole transaction. The restricted publication retains server-timed approvals, quarantined scope identities, and audience-safe migration instructions. Unsupported queue, session, workflow, release, or credential pause claims are rejected until their authoritative stores provide enforcement adapters. Independently controlled forks and federated copies remain `awaiting_owner` and must rewrite under their own authority.
Containment passes continuously re-evaluate a closed ten-part completion policy for authoritative refs, ordinary object access, fork/federation acknowledgement, package/artifact replacement, credential rotation, deployments, caches, and recovery copies. Repository reachability and quarantine are server-derived; quarantined Git objects must be unreachable from every advertised ref, and upload-pack disables direct or reachable SHA wants. Every other observation retains current SHA-256 evidence. Only the latest complete passing pass can restore explicitly named push, automation, release, or contribution flows; a retained push pause remains enforced regardless of the publication display state. Each pull/workspace migration target binds one selected candidate ref at publication, derives its exact replacement revision server-side, and accepts preserved discussion/attribution only for that same resource/ref/revision tuple. Legal retention, independent or unreachable copies, reintroduction, and maximum-30-day exceptions remain visible blockers, and the ledger never emits an erasure claim.
...
from only the search's frozen scenario to candidates and derives recommended trials, ancestry-proven competing
working-to-regressed ranges, confidence, direct diffs,
...
- **Reproducible learning attempts** — Authenticated repository readers launch
  ...
  A durable filesystem claim spans lookup, comparison, and creation across API
  workers sharing the workspace root; per-attempt durable claims likewise span
  provisioning, so correctness does not depend on one process's mutexes.
