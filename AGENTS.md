...
Live runbook recommendations and executions at repository `/runbook-recommendations` and `/runbook-executions` routes freeze an alert, incident, deployment, failed workflow, service objective, support thread, or manual observation together with its exact revision, affected resources, signal window, releases, environment snapshot, digest-addressed permitted evidence, timeline references, and audience. Explained scope matches never launch automatically; equal matches require an explicit choice. A caller-stable launch binds one immutable runbook revision and retains explicit precondition and current-access decisions as either a `ready` or `blocked` execution. Missing current rehearsal, stale procedure, blocking runbook diagnostics, unmet preconditions, unavailable access, duplicate active origin, ambiguous matches, and secret-bearing or incomplete context fail visibly. Executions coordinate guided response only and grant none of the referenced repository, evidence, environment, workflow, deployment, disclosure, or operational authority.
