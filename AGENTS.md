...
After a complete passing rehearsal, a named human response owner can publish only the retained candidate with every required role threshold satisfied. Git's ref transaction compare-and-swaps all selected old tips to attested replacements atomically; exact retries reconcile already-moved refs, while any independently moved ref aborts the whole transaction. The restricted publication retains server-timed approvals, quarantined scope identities, paused restoration paths, and audience-safe migration instructions. Ordinary pushes receive recovery guidance while migration is active. Independently controlled forks and federated copies remain `awaiting_owner` and must rewrite under their own authority.
